...
$array_cod_compras = json_decode($_POST['array_cod_compras']);
	...
	buscarListaPagos($codProveedorPago,$array_cod_compras);
...
$codProveedorPago = utf8_decode($codProveedorPago);
	...
	buscarHistorialPagos($codProveedorPago);
...
$cod_chequeListadoPago = utf8_decode($cod_chequeListadoPago);
	EliminarListadoPago($cod_chequeListadoPago);
...
buscarCompraProveedorDetalle($fecha1,$fecha2,$proveedor);
...
$fecha2 = utf8_decode($fecha2);
	$proveedor=$_POST['proveedor'];
$proveedor = utf8_decode($proveedor);
buscarCompraProveedor($fecha1,$fecha2,$proveedor);
...
function buscarCompraProveedor($fecha1,$fecha2,$proveedor)
{
	...
	$mysqli=conectar_al_servidor();
		...
		$sql= "Select cod_compra,fecha_compra,cod_proveedorFK,num_comprobante,cod_local, sum(descuento) as descuento,pagado1,pagado2,
		(Select nombre_persona from persona where cod_persona=cod_proveedorFK) as proveedor, 
		(Select nombre_persona from persona where cod_persona=cod_user_insert) as usuario, tipotr,
		(Select Nombre from local l where l.cod_local=cp.cod_local) as nombrelocal, 
		sum(IFNULL((select sum(subTotal) from detalle_compra where cod_compra=cod_compraFK ),0)) as totalcompra,
		sum(IFNULL((select sum(subTotal) from detalle_compra where cod_compra=cod_compraFK and (select tipotr from compra where cod_compra=cod_compraFk)='NOTA DE CREDITO' ),0)) as totalNC,
	    sum(IFNULL((select sum(importe) from cheque where cod_compra=cod_compraFk ),0)) as totalpagado,
		sum(IFNULL((select sum(importe) from cheque where cod_compra=cod_compraFk ),0)) as totalpagos,
				cp.fecha_insert,cp.fecha_edit,
		(Select nombre_persona from persona pra where pra.cod_persona=cod_user_insert )as insertadopor,
		(Select nombre_persona from persona pra where pra.cod_persona=cod_user_edit )as editadopor
		from compra cp  where  cp.estado='Activo' and cod_compra!='0'  ".$condicionFecha.$condicionproveedor.$condicionpagos." group by  cod_proveedorFK asc ";
 ...
 if ($valor>0)
 {
	  while ($valor= mysqli_fetch_assoc($result))
	  {
		      ...
		      $fecha_compra=$valor['fecha_compra'];
		  	  ...
		  	  $proveedor=utf8_encode($valor['proveedor']);
		  	  $subtotalcompra=utf8_encode($valor['totalcompra']);
			  $totalNC=utf8_encode($valor['totalNC']);
			  
		  	  $cod_local=utf8_encode($valor['cod_local']);
		  	  $nombrelocal=utf8_encode($valor['nombrelocal']);
		  	 $descuento=utf8_encode($valor['descuento']);
			  $totalpagado=utf8_encode($valor['totalpagado']);
			   $tipotr=utf8_encode($valor['tipotr']);
...
$usuario = utf8_encode($valor['usuario']); 
	    ...
	    $totalNC= $totalNC * -1 ;
		$totalPendiente = $subtotalcompra -  $totalpagado;
 		$subtotalcompra = $subtotalcompra + $totalNC ;
		
		$totalPendiente= $totalPendiente +  $totalNC ;
...
<td  id='td_datos_9' style='display:none'>".$condicionpagos."</td>
</tr>
</table>";
 ...
 mysqli_close($mysqli);   
$informacion =array("1" => "exito","2" => $pagina,"3" => $nroRegistro,"4"=>number_format($totalCompas,'0',',','.'),"5"=>number_format($totalDesc,'0',',','.'),"6"=>number_format($TotalesPendiente,'0',',','.'),"7"=>number_format($TotalesPagago,'0',',','.'));
echo json_encode($informacion);	
exit;
}

function buscarCompraProveedorDetalle($fecha1,$fecha2,$proveedor)
{
...
(Select nombre_persona from persona pra where pra.cod_persona=cod_user_insert )as insertadopor,
(Select nombre_persona from persona pra where pra.cod_persona=cod_user_edit )as editadopor
		from compra cp  where  cp.estado='Activo'  ".$condicionFecha.$condicionproveedor.$condicionpagos." order by  fecha_compra asc ";
...
$descuento = utf8_encode($valor['descuento']); 
 ...
 if($tipotr=="NOTA DE CREDITO"){
	 $subtotalcompra= $subtotalcompra * -1 ;
 }
 
	$subtotalcompra= $subtotalcompra - $descuento ;
 
 $accion="<input id='check".$cod_compra."' value='".$cod_compra."'  name='".number_format($subtotalcompra,'0',',','.')."' type='checkbox' onclick='verificarConfirmarPagoProveedor(this)' checked />";
 
 $Monto="<input class=''  id='input".$cod_compra."' value='".number_format($subtotalcompra,'0',',','.')."' type='text'  style='width:135px;border:none;opacity: 0.5;' disabled>";
 
 $ver=" <center><input type='button' value='VER' class='btn4' onclick='verCerrarAbmPagoProveedorDetalleCompra(".$cod_compra.")' id='".$cod_compra."' />  </center>";
...
<td  id='td_datos_10' style='display:none'>".$cod_compra."</td>
 ...
 mysqli_close($mysqli);   
$informacion =array("1" => "exito","2" => $pagina,"3" => $nroRegistro,"4"=>number_format($totalCompas,'0',',','.'),"5"=>number_format($totalDesc,'0',',','.'));
echo json_encode($informacion);	
exit;
}
...
function buscarListaPagos($codProveedorPago,$array_cod_compras)
{
...
on ch.idcheque = dpc.idchequeFK where dpc.cod_compraFK = '$array_cod_compras[0]' and ch.estado = 'Activo'";
...
<td  id='td_datos_2' style='display:none'>".$idcheque."</td>
  ...
  $informacion =array("1" => "exito","2" => $pagina,"3" => number_format($total_pagado,'0',',','.'));
echo json_encode($informacion);	
exit;

}
function buscarHistorialPagos($codProveedorPago)
{
...
function EliminarListadoPago($cod_chequeListadoPago)
{	
