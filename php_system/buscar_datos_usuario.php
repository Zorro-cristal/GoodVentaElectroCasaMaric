...
function buscardatos($user)
{
	$mysqli=conectar_al_servidor();
	 $pagina='';
		$sql= "Select pr.nombre_persona,us.acceso,us.cod_localFK,us.foto_perfil,us.login,
		IFNULL((Select cdu.cod_cobradorFk from cobradorusuario cdu where cdu.cod_usuarioFk=us.cod_usuario),0) as ControlCobra
		from  persona pr inner join usuario us on us.cod_usuario=pr.cod_persona  where cod_persona=? and us.estado='Activo' ";
...
if ( ! $stmt->execute()) {
 ...
 if ($valor>0)
 {
	  while ($valor= mysqli_fetch_assoc($result))
	  {
		  	  ...
		  	  if ($foto_perfil !== '') {
		  	  	$archivoFoto = __DIR__.DIRECTORY_SEPARATOR.'..'.DIRECTORY_SEPARATOR.str_replace('/', DIRECTORY_SEPARATOR, $foto_perfil);
		  	  	if (!is_file($archivoFoto)) $foto_perfil = '';
		  	  }
		  	  $login=utf8_encode($valor['login']);
		   $accesos=buscaracceso($user);
		  
		  $informacion =array("1" =>"exito","2" => $nombre_persona,"3" => $acceso,"4" => $cod_localFK ,"5" => $accesos,"6" => $ControlCobra ,"7" => $ControlCobra,"8" => $foto_perfil,"9" => $login );
echo json_encode($informacion);	
exit;
