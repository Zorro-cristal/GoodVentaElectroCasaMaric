...
$fechaDeposito = utf8_decode($fechaDeposito);
	...
	abm($fechaDeposito,$cod_motivo,$Arreglo,$nroboleta, $banco , $nrocuenta ,$idgastos,$monto,$motivo,$fecha,$estado,$personales,$cod_usuario,$cod_local,$tipo,$codcaja,$idaperturacierrecaja,$extgasto,$operacion);
...
if($operacion=="buscarSeleccionarBusquedaEgresosTotalLocal")
{
	...
	buscarSeleccionarBusquedaEgresosTotalLocal($formato);
...
if ($operacion == "buscar_total_egresos_local_general") {
		...
		$formato=isset($_POST['formato']) ? utf8_decode($_POST['formato']) : '';
		buscar_total_egresos_local_general($anho, $local, $array_cod_seleccionar_busqueda_egresos_total_local,$formato);
	}
...
if ($operacion == "buscar_total_egresos_local_general_grafica") {
	...
	if ($operacion == "buscar_total_egresos_local_general_incremental") {
		...
		$array_cod_seleccionar_busqueda_egresos_total_local = json_decode($_POST['array_cod_seleccionar_busqueda_egresos_total_local']);
		$formato=isset($_POST['formato']) ? utf8_decode($_POST['formato']) : '';
		buscar_total_egresos_local_general_incremental($anho, $local,$array_cod_seleccionar_busqueda_egresos_total_local,$formato);
	}
if($operacion=="buscar")
{
	$fecha1=$_POST['fecha1'];
...
buscar($permisover,$user,$arreglo,$fecha1,$fecha2,$estado,$cod_local,$tipo,$usuario,$fecha,$motivo,$confirmado,$agrupacionformulariogasto,$banco,$nroboleta,$monto,$busqueda_general);
...
function abm($fechaDeposito,$cod_motivo,$Arreglo,$nroboleta, $banco , $nrocuenta,$idgastos,$monto,$motivo,$fecha,$estado,$personales,$cod_usuario,$cod_local,$tipo,$codcaja,$idaperturacierrecaja,$extgasto,$operacion)
{
...
$consulta1="Insert into gastos (arreglo,monto,motivo,fecha,estado,cod_usuario,personales,cod_local,tipo,codCaja,codApertura,nroboleta,banco,nrocuenta,cod_motivo,fechaDeposito)
values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
$stmt1 = $mysqli->prepare($consulta1);
$ss='ssssssssssssssss';
$stmt1->bind_param($ss,$Arreglo,$monto,$motivo,$fecha,$estado,$cod_usuario,$personales,$cod_local,$tipo,$codcaja,$idaperturacierrecaja,$nroboleta, $banco , $nrocuenta,$cod_motivo,$fechaDeposito);
...
personales=?,cod_local=?,tipo=?,nroboleta=?,banco=?,nrocuenta=?,cod_motivo=?,fechaDeposito='$fechaDeposito' where idgastos=?";
$stmt1 = $mysqli->prepare($consulta1);
$ss='sssssssssssss';
$stmt1->bind_param($ss,$Arreglo,$monto,$motivo,$fecha,$estado,$personales,$cod_local,$tipo,$nroboleta,$banco,$nrocuenta,$cod_motivo,$idgastos); 
...
$ruta="/GoodVentaElectroCasaMaric/fotos/gastos/".$id_foto.$id_f.'.'.$ext;
...
function buscar($permisover,$user,$arreglo,$fecha1,$fecha2,$estado,$cod_local,$tipo,$usuario,$fecha,$cod_motivo,$confirmado,$agrupacionformulariogasto,$banco,$nroboleta,$monto,$busqueda_general='')
{
	$mysqli=conectar_al_servidor();
	 $pagina='';
	 $filas=array();
	 ...
	 $condicionCodLocal=" and g.cod_local='$cod_local' ";
		 ...
		 $condicionbanco="";
		 if($banco!=""){
			$condicionbanco=" and banco like '%".$banco."%' "; 
		 }
		 ...
		 if($busqueda_general!=""){
			...
			$condicionBusquedaGeneral=" and (
				ifnull((select descripcion from motivo_e_i where idmotivo_e_i=g.cod_motivo),'') like '%".$busquedaGeneralSegura."%'
				or g.motivo like '%".$busquedaGeneralSegura."%'
				or g.tipo like '%".$busquedaGeneralSegura."%'
				or g.nroboleta like '%".$busquedaGeneralSegura."%'
				or g.banco like '%".$busquedaGeneralSegura."%'
				or g.nrocuenta like '%".$busquedaGeneralSegura."%'
				or g.arreglo like '%".$busquedaGeneralSegura."%'
				or (select nombre_persona from persona where cod_persona=g.cod_usuario) like '%".$busquedaGeneralSegura."%'
				or (select Nombre from local where cod_local=g.cod_local) like '%".$busquedaGeneralSegura."%'
			) ";
		 }
		 
		 $condiciones_busqueda_gasto_balance = $condicionpermisover.$condicionCodLocal.$condicionarreglo.$condiciontipo.$condicionfecha.$condicionusuario.$condicionrangofechas.$condicionmotivo.$condicionconfirmado.$condicionbanco.$condicionnroboleta.$condicionmonto.$condicionBusquedaGeneral;
		 $resumen_gastos_depositos_balance = obtener_resumen_gastos_depositos_balance($mysqli, $estado, $condiciones_busqueda_gasto_balance);
		 
		$sql= "Select fechaDeposito,arreglo,monto, ifnull((select descripcion from  motivo_e_i where idmotivo_e_i=cod_motivo),'') as descripcion,motivo,fecha,estado,cod_usuario,idgastos,tipo,cod_local,nroboleta,banco,nrocuenta,cod_motivo,
		(Select nombre_persona from persona where cod_persona=cod_usuario) as usuarionombre,confirmado,url,
		(SELECT idbanco FROM banco b where b.nombre = g.banco) as cod_banco,
		(Select Nombre from local l where l.cod_local=g.cod_local) as nombrelocal
		from gastos g where  estado='$estado' ".$condicionpermisover.$condicionCodLocal.$condicionarreglo.$condiciontipo.$condicionfecha.$condicionusuario.$condicionrangofechas.$condicionmotivo.$condicionconfirmado.$condicionbanco.$condicionnroboleta.$condicionmonto.$condicionBusquedaGeneral.$condicionagrupacion;
...
if ( ! $stmt->execute()) {
 ...
 if ($valor>0)
 {
	  while ($valor= mysqli_fetch_assoc($result))
	  {
			  ...
			  $cod_banco=utf8_encode($valor['cod_banco']);
			  ...
			  $marcaReferenciaPago=" | COMP. PAGO:";
			  $posReferenciaPago=strpos($motivoVisible,$marcaReferenciaPago);
			  if($posReferenciaPago!==false){
				  $motivoPrincipal=substr($motivoVisible,0,$posReferenciaPago);
				  $referenciaPago=substr($motivoVisible,$posReferenciaPago+3);
				  $motivoVisible=$motivoPrincipal."<br><b>".$referenciaPago."</b>";
			  }else{
				  if(strpos($motivoVisible,"COMP. PAGO:")===0){
					  $motivoVisible="<b>".$motivoVisible."</b>";
				  }
			  }
			  if($agrupacionformulariogasto == '2'){
				  ...
				  $MotivoGrupo=$motivoVisible." <br><b>".$descripcion."</b>";
			  }
			  $btnVer = '';
			  $fotourl = '';
			  $ext = '';
...
if($tipo!="Deposito"){
	$fechaDeposito="";
			  ...
			  $montoListado=$agrupacionformulariogasto == '2' ? $totalMonto : $monto;
			  $filas[]=array(
				  "id" => $idgastos,
				  "motivo" => $motivo,
				  "descripcion" => $descripcion,
				  "motivo_orden" => $agrupacionformulariogasto == '2' ? $descripcion : trim($motivo.' '.$descripcion),
				  "monto" => floatval($montoListado),
				  "monto_formateado" => number_format($montoListado,'0',',','.'),
				  "tipo" => $tipo,
				  "fecha_deposito" => $fechaDeposito,
				  "fecha" => $fecha,
				  "fecha_formateada" => $fecha2,
				  ...
				  "foto_url" => $fotourl,
				  "foto_ext" => $ext,
				  "agrupado" => $agrupacionformulariogasto == '2' ? 'SI' : 'NO'
			  );
...
<td  id=''	style='width:5%'>".$banco."</td>
<td  id='td_datos_9'	style='display:none'>".$cod_banco."</td>
...
$informacion =array(
	...
	"3" => $nroRegistro,
	"4" =>  number_format($agrupacionformulariogasto == '2' ? $totalMontoAgrupado : $totalGasto,'0',',','.'),
	"5" => number_format($resumen_gastos_depositos_balance["total_suma"],'0',',','.'),
	"6" => number_format($resumen_gastos_depositos_balance["total_egreso"],'0',',','.'),
	"7" => number_format($resumen_gastos_depositos_balance["total_neto"],'0',',','.'),
	"8" => number_format($resumen_gastos_depositos_balance["total_deposito"],'0',',','.'),
	"9" => number_format($resumen_gastos_depositos_balance["total_ingreso"],'0',',','.'),
	"10" => $resumen_gastos_depositos_balance["registros_suma"],
	"11" => $resumen_gastos_depositos_balance["registros_egreso"]
);
echo json_encode($informacion);	
exit;
...
function obtener_resumen_gastos_depositos_balance($mysqli, $estado, $condiciones)
{
	$resumen = array(
		"total_deposito" => 0,
		"total_ingreso" => 0,
	...
	$sql = "SELECT 
		IFNULL(SUM(CASE WHEN tipo='Deposito' THEN monto ELSE 0 END),0) AS total_deposito,
		IFNULL(SUM(CASE WHEN tipo='Ingreso' THEN monto ELSE 0 END),0) AS total_ingreso,
		IFNULL(SUM(CASE WHEN tipo='Egreso' THEN monto ELSE 0 END),0) AS total_egreso,
		IFNULL(SUM(CASE WHEN tipo IN ('Deposito','Ingreso') THEN 1 ELSE 0 END),0) AS registros_suma,
		IFNULL(SUM(CASE WHEN tipo='Egreso' THEN 1 ELSE 0 END),0) AS registros_egreso
	FROM gastos g
	WHERE estado='$estado' ".$condiciones;
	...
	if ($valor = mysqli_fetch_assoc($result)) {
		$resumen["total_deposito"] = floatval($valor["total_deposito"]);
		$resumen["total_ingreso"] = floatval($valor["total_ingreso"]);
		$resumen["total_egreso"] = floatval($valor["total_egreso"]);
		$resumen["total_suma"] = $resumen["total_deposito"] + $resumen["total_ingreso"];
		$resumen["total_neto"] = $resumen["total_suma"] - $resumen["total_egreso"];
		$resumen["registros_suma"] = intval($valor["registros_suma"]);
		$resumen["registros_egreso"] = intval($valor["registros_egreso"]);
	}
...
$condicionCodLocal=" and g.cod_local='$cod_local' ";
		...
		$sql= "Select arreglo,monto, ifnull((select descripcion from  motivo_e_i where idmotivo_e_i=cod_motivo),'') as descripcion,motivo,fecha,estado,cod_usuario,idgastos,tipo,cod_local,nroboleta,banco,nrocuenta,cod_motivo,
		(Select nombre_persona from persona where cod_persona=cod_usuario) as usuarionombre,confirmado,
		(Select Nombre from local l where l.cod_local=g.cod_local) as nombrelocal
...
$informacion =array("1" => "exito","2" => ($formato=='json' ? $filas : $pagina),"3" => number_format($nroRegistro,'0',',','.'),"4" => number_format($totalGasto,'0',',','.'));
echo json_encode($informacion);	
exit;
...
$filas[]=array(
	...
	"latitud" => $lat,
	"longitud" => $lot,
	"numero_factura_pago" => $nrofactura
);
...
$informacion =array("1" => "exito","2" => ($formato=='json' ? $filas : $pagina),"3" => number_format($nroRegistro,'0',',','.'),"4" => number_format($totalPagado,'0',',','.'));
echo json_encode($informacion);	
exit;
}
...
<td  id='' style='width:10%'>".$nombrelocal."</td>
 ...
 $informacion =array("1" => "exito","2" => ($formato=='json' ? $filas : $pagina),"3" => number_format($nroRegistro,'0',',','.'),"4" => number_format($total_compra,'0',',','.'));
echo json_encode($informacion);	
exit;
...
$informacion =array("1" => "exito","2" => ($formato=='json' ? $filas : $pagina),"3" => number_format($nroRegistro,'0',',','.'),"4" => number_format($totalventas,'0',',','.'));
echo json_encode($informacion);	
exit;
}
 ...
 if ($valor>0)
 {
	  while ($valor= mysqli_fetch_assoc($result))
	  {
			  ...
			  $filas[]=array(
				  "codigo" => $idmotivo_e_i,
				  "descripcion" => $descripcion,
				  "estado" => utf8_encode(isset($valor['Estado']) ? $valor['Estado'] : $valor['estado'])
			  );
...
if($observacion !=''){
	...
	$verObs = "<input type=\"button\" value=\"OBS\" style=\"width:50px;background-color:#ff5733\" class=\"btn4\" onclick=\"verObservacionIngresoEgresoCobrador('$obsEscapado')\" />";
		...
		$fecha2 = date("d-m-Y", strtotime($fecha));
		$montoMostrar = $agrupacionformulariogastoscobrador == '2' ? $totalMonto : $monto;
		$filas[]=array(
			"codigo" => $idgastos_cobrador,
			"motivo" => $motivo,
			"monto" => (float)$montoMostrar,
			"monto_formateado" => number_format($montoMostrar,'0',',','.'),
			"tipo" => $tipo,
			"fecha_visible" => $fecha2,
			"fecha" => $fecha,
			"numero_boleta" => $nroboleta,
			"banco" => $banco,
...
/*Retornamos los datos obtenidos mediante el JSON */      
$informacion =array("1" => "exito","2" => ($formato=='json' ? $filas : $pagina),"3" => $nroRegistro,"4" => number_format($totalGasto,'0',',','.'),"5" => number_format($totalIngreso,'0',',','.'));
echo json_encode($informacion);	
exit;
...
function buscar_total_egresos_local_general($anho,$local,$array_cod_seleccionar_busqueda_egresos_total_local,$formato='')
{
	...
	for($x = 1; $x <= 31; $x++){
		...
		for ($i = 1; $i <= 12; $i++) {
			...
			$pagina.= $td;
			$meses[] = number_format($total, '0', ',', '.');
		}
		
		$pagina.="</tr>
		</table>";
		$filas[]=array('dia'=>$x,'meses'=>$meses,'clase_fila'=>$styleName);
	}
...
function buscar_total_egresos_local_general_incremental($anho,$local,$array_cod_seleccionar_busqueda_egresos_total_local,$formato='')
{
	...
	$informacion = array("1" => "exito", "2" => ($formato==='json' ? $filas : $pagina),"3"=>$array_total_egresos_local,"4"=>$array_colores);
	echo json_encode($informacion);
	exit;
}
...
function buscarSeleccionarBusquedaEgresosTotalLocal($formato='')
{
	$pagina = '';
	$filas = array();
 $styleName="tableRegistroSearch";
...
foreach ($elementosBusqueda as $clave => $valor) {
	...
	$styleName=CargarStyleTable($styleName);
	$filas[]=array('id'=>$clave,'descripcion'=>$valor,'clase_fila'=>$styleName);
	$pagina.="
		<table class='$styleName' border='1' cellspacing='1' cellpadding='5'>
			<tr id='tbSelecRegistro'>
  ...
  $informacion =array("1" => "exito","2" => ($formato==='json' ? $filas : $pagina),"3"=>count($elementosBusqueda));
echo json_encode($informacion);	
exit;
}
