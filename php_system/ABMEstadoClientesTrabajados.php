...
if($funt=="nuevo" || $funt=="editar")
{
	...
	abm($idabm,$descripcion,$Estado,$funt);
...
function abm($idestados_clientes_trabajados,$descripcion,$Estado,$funt)
{
...
exit;
	...
	if($funt=="nuevo")
	{
				$consulta= "Select count(*) from estados_clientes_trabajados where descripcion=? and estado ='Activo' ";
...
if($valor==1)
{
    ...
    $consulta="insert into estados_clientes_trabajados (descripcion,estado) values (UPPER(?),?)";	
     $stmt = $mysqli->prepare($consulta);
    $ss='ss';
    ...
    $consulta="Update estados_clientes_trabajados set descripcion=UPPER(?),estado=? where idestados_clientes_trabajados=?";	
    ...
    $stmt->bind_param($ss,$descripcion,$Estado,$idestados_clientes_trabajados); 
...
function buscar($buscar,$Estado)
{
	$mysqli=conectar_al_servidor();
	 ...
	 $formato=isset($_POST['formato']) ? $_POST['formato'] : '';
		$sql= "Select idestados_clientes_trabajados,descripcion,estado
        from estados_clientes_trabajados where descripcion like ?  and estado=? order by descripcion asc ";
...
if ( ! $stmt->execute()) {
 ...
 if ($valor>0)
 {
	  while ($valor= mysqli_fetch_assoc($result))
	  {
		      ...
		      $idestados_clientes_trabajados=$valor['idestados_clientes_trabajados'];
			  $descripcion=utf8_encode($valor['descripcion']);
			  $Estado=utf8_encode($valor['estado']);
			  $filas[]=array(
				  "codigo" => $idestados_clientes_trabajados,
			  ...
			  <tr id='tbSelecRegistro' onclick='ObtenerdatosAbmEstadosClientesTrabajados(this)'>
			  <td id='td_id' style='display:none;'>".$idestados_clientes_trabajados."</td>
...
function buscarOption()
{
	$mysqli=conectar_al_servidor();
	 $pagina="<option value='' >SELECCIONAR</option>";  
		$sql= "Select idestados_clientes_trabajados,descripcion,estado
        from estados_clientes_trabajados where estado='Activo' order by descripcion asc ";
