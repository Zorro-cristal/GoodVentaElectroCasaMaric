...
$buscar = utf8_decode($buscar);
$formato=isset($_POST['formato']) ? $_POST['formato'] : '';
buscardetalles($idAbmListaNiveles,$buscar,$formato);
...
if($funt=="editaraccesosmasivo")
{
$ids=json_decode(isset($_POST['ids']) ? $_POST['ids'] : '', true);
$idAbmListaNiveles=isset($_POST['idAbmListaNiveles']) ? utf8_decode($_POST['idAbmListaNiveles']) : '';
$acciones=isset($_POST['acciones']) ? utf8_decode($_POST['acciones']) : '';
editaraccesosmasivo($ids,$idAbmListaNiveles,$acciones);
}
...
function editaraccesosmasivo($ids,$cod_nivelesfk,$acciones)
{
	if(!is_array($ids) || count($ids)==0 || $cod_nivelesfk=='' || ($acciones!='SI' && $acciones!='NO')){
		$informacion=array("1" => "DI");
		echo json_encode($informacion);
		exit;
	}
	...
	$consulta="Update detallesniveles set accion=? where iddetallesniveles=? and cod_nivelesfk=?";
	$stmt=$mysqli->prepare($consulta);
	if(!$stmt){
		$mysqli->rollback();
		$informacion=array("1" => "error");
	...
	foreach($idsValidos as $iddetallesniveles){
		$ss='sis';
		$stmt->bind_param($ss,$acciones,$iddetallesniveles,$cod_nivelesfk);
		if(!$stmt->execute()){
			$stmt->close();
			$mysqli->rollback();
			$mysqli->close();
	...
	$mysqli->commit();
	$mysqli->close();
	$informacion=array("1" => "exito","2" => count($idsValidos));
	echo json_encode($informacion);
	exit;
}
...
function buscardetalles($idAbmListaNiveles,$buscar,$formato='')
{
	$mysqli=conectar_al_servidor();
	 $pagina='';
	 $filas=array();
		...
		where cod_nivelesfk='".$idAbmListaNiveles."' and  concat(lta.nombre,' ',lta.formulario) like '%".$buscar."%' order by lta.nro asc,lta.orden asc";
 ...
 if ($valor>0)
 {
	  while ($valor= mysqli_fetch_assoc($result))
	  {
		     ...
		     $iddetallesniveles=$valor['iddetallesniveles'];
			  ...
			  $filas[]=array(
				"iddetallesniveles" => $iddetallesniveles,
				"nro" => $nro,
