...
$formato = isset($_POST['formato']) ? utf8_decode($_POST['formato']) : "";
	buscar($fechaEmi,$NroCheque,$fechaven,$orden,$concepto,$pago,$banco,$Fecha1,$Fecha2,$tipo,$monto,$cod_cheque,$formato);
...
if($funt=="NuevoConceptoChequeACobrar")
{
	$concepto=$_POST['concepto'];
$concepto = utf8_decode($concepto);
...
function buscaroptionConcepto()
{
	...
	$filas=array();
		...
		$sql= "Select * from concepto_chequeacobrar where estado='Activo' order by descripcion asc  ";
...
if ( ! $stmt->execute()) {
 ...
 if ($valor>0)
 {
	  while ($valor= mysqli_fetch_assoc($result))
	  {
		      ...
		      $idconcepto_cheque=$valor['idconcepto_chequeacobrar'];
...
$consulta1="Insert into concepto_chequeacobrar (descripcion,estado) values (upper(?),'Activo')";
...
if($pagado=="PENDIENTE"){
	...
	$consulta= "Select count(*) from chequeacobrar where nombre_banco=? and nroche=?  ";
...
if($valor>0)
{
    ...
    $consulta="insert into chequeacobrar ( fecemi, nroche, fecven, orden, concep, importe, pagado, nombre_banco,estado,tipo) values (?,?,?,upper(?),upper(?),?,?,?,?,?)";	
     $stmt = $mysqli->prepare($consulta);
    $ss='ssssssssss';
    ...
    $consulta="Update chequeacobrar set fecemi='$fechaemi', nroche='$nroCheque', fecven='$fechaven', orden=upper('$orden'), concep=upper('$concepto'), importe=$importe, pagado=$pagado, nombre_banco='$banco',estado='$estado',tipo='$tipo' where idchequeacobrar=$idAbmCheque";	
...
if($respuesta){
	$mysqli=conectar_al_servidor();
	$consulta="UPDATE chequeacobrar SET url = '$ruta' WHERE idchequeacobrar  = '$idcheque'";	
	$stmt = $mysqli->prepare($consulta);
if ( ! $stmt->execute()) {
   echo "Error";
   exit;
...
if($pagado=="PENDIENTE"){
	...
	$consulta= "Select count(*) from chequeacobrar where nombre_banco='$banco' and nroche='$nroCheque'";
$stmt = $mysqli->prepare($consulta);
...
if($valor>0)
{
    ...
    $consulta="insert into chequeacobrar ( fecemi, nroche, fecven, orden, concep, importe, pagado, nombre_banco,estado,tipo,ingresadopor_cod_user,fechaingresado) values (?,?,?,upper(?),upper(?),?,?,?,?,?,?,?)";	
     $stmt = $mysqli->prepare($consulta);
    $ss='ssssssssssss';
...
function obtenerultimaidcheque()
{
	...
	$mysqli=conectar_al_servidor();
	 $pagina='';
	 $sql= "Select idchequeacobrar from chequeacobrar where estado='Activo'  order by idchequeacobrar desc limit 1";
 ...
 if ($valor>0)
 {
	  while ($valor= mysqli_fetch_assoc($result))
	  {
		      ...
		      $idchequeacobrar=$valor['idchequeacobrar'];
...
function buscar($fechaEmi,$NroCheque,$fechaven,$orden,$concepto,$pago,$banco,$Fecha1,$Fecha2,$tipo,$monto,$cod_cheque,$formato)
{
	$mysqli=conectar_al_servidor();
		...
		if($fechaEmi!=""){
		 ...
		 if($concepto!=""){
		  ...
		  $condicioncodcheque="";
		 if($cod_cheque!=""){
			$condicioncodcheque=" and idchequeacobrar = '$cod_cheque' "; 
		 }
		 ...
		 $condicionbanco="";
		 if($banco!=""){
			$condicionbanco=" and nombre_banco = '".$banco."' "; 
		 }
	 ...
	 $pagina='';
	 $filas=array();
		$sql= "Select idchequeacobrar, fecemi, nroche, fecven, orden, concep, importe, pagado, nombre_banco ,url, estado,tipo,
		nombre_banco as banco
        from chequeacobrar where  estado='Activo' ".$condicionfechaEmi.$condicionNroCheque.$condicionfechaven.$condicionorden.$condicionconcepto.$condicionpago.$condicionbanco.$condicionrangofechas.$condicionmonto.$condiciontipo.$condicioncodcheque." order by fecven desc limit 500 ";
 ...
 if ($valor>0)
 {
	  while ($valor= mysqli_fetch_assoc($result))
	  {
			  ...
			  $importe=$valor['importe'];
		  	  ...
		  	  $nombre_banco=utf8_encode($valor['nombre_banco']);
...
<tr id='tbSelecRegistro' onclick='ObtenerdatosAbmChequeACobrar(this)'>
<td id='td_id' style='width:5%; background-color: #efeded;color:red'>".$idchequeacobrar."</td>
...
<td  id='td_datos_9' style='display:none'>".$nombre_banco."</td>
...
function buscarpagadoproveedor($buscar)
{
	$mysqli=conectar_al_servidor();
	 ...
	 $pagina='';
		$sql= "Select idchequeacobrar, fecemi, nroche, fecven, orden, concep, importe, pagado, nombre_banco , estado,tipo,
		nombre_banco as banco
        from chequeacobrar where  estado='Activo' and cod_compraFK = '$buscar' order by fecven desc limit 500 ";
...
function buscarInformePagosAProveedorCheques($idpago_total_compra)
{
	$mysqli=conectar_al_servidor();
	 $pagina='';
		...
		$sql= "SELECT nroche, orden, concep, importe,nombre_banco as banco, (SELECT nombre_persona FROM persona WHERE cod_persona = cod_proveedorFK) as proveedor,fecemi FROM detalle_pago_cheque dpc INNER JOIN chequeacobrar ch ON dpc.idchequeFK = ch.idchequeacobrar  WHERE idpago_total_compraFK = '$idpago_total_compra' and ch.estado = 'Activo' GROUP BY idchequeFK";
