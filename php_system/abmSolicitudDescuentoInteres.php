...
if($operacion=="nuevo" )
{
	...
	abm($cod_ventaFK,$CodUsu,$cod_creditoFK,$monto,$cod_localFK,$totalInteres,$cuota_nro,$motivo,$cod_clienteFK,$diasatraso,$montocuota,$operacion);
...
if($operacion=="buscarSoliDescuento")
{
	...
	buscarSoliDescuento($fecha1,$fecha2,$UsuSoli,$UsuApro,$producto,$formato);
...
if($operacion=="buscarInforme")
{
	...
	$estado_uso = utf8_decode($estado_uso);
	...
	buscarInforme($fecha1,$fecha2,$usuario,$motivo,$cod_credito,$estado_uso,$estado,$cliente,$formato);
...
if($estado == 'APROBADO'){
	actualizarCreditoDescuentoInteres($idABM);
}
...
function actualizarCreditoDescuentoInteres($iddescuentoInteres){
$mysqli=conectar_al_servidor();


	$sql= "SELECT * FROM solicituddescuentointeres WHERE idsolicituddescuentointeres = '$iddescuentoInteres'";	
...
{  
	...
	$tipo_pago = utf8_encode($valor['tipo_pago']); 
}
}


	if($tipo_pago == 'NORMAL'){
		$sql= "SELECT IFNULL(descuentoInteres,0) as descuentoInteres FROM credito WHERE idcredito = '$cod_creditoFK'";
			...
			if($descuentoInteres > 0){
				$totalaprobado += $descuentoInteres;
			}
		
			$sql= "UPDATE credito SET descuentoInteres = '$totalaprobado' WHERE idcredito = '$cod_creditoFK'";	
	...
	if($tipo_pago == 'PARCIAL'){
			...
			$sql= "Select (totalinteres + cr.deudaInteres) as totalinteres,cr.idcredito,cr.plazo,IFNULL(descuentoInteres,0) as descuentoInteres
					from credito cr inner join venta vt on vt.cod_venta=cr.cod_venta
					where cr.cod_venta='$cod_ventaFK' and IFNULL((select sum(pg.Monto) from pago pg where pg.cod_creditoFK=cr.idcredito and tipo='Pago Cuota'),0) < (cr.Monto-cr.descuento)  order by cr.idcredito asc";
			...
			if ($valor>0)
			{
				...
				{  
					...
					$restante = $totalinteres - $descuentoInteres;
					...
					if ($totalaprobado >= $restante) {
						
						actualizardescuentointeres($idcredito, $descuentoInteres + $restante);
						
						$totalaprobado -= $restante;
					} else {
						
						actualizardescuentointeres($idcredito, $descuentoInteres + $totalaprobado);
						$totalaprobado = 0;
						break;
					}
				}
			...
			$consulta1="update solicituddescuentointeres set estado_uso = 'FINALIZADO' where cod_ventaFK= $cod_ventaFK and estado_uso = 'PENDIENTE' and tipo_pago = 'PARCIAL'";
...
function actualizardescuentointeres($idcredito,$monto){
	$mysqli=conectar_al_servidor();
	$sql= "UPDATE credito SET descuentoInteres = '$monto' WHERE idcredito = '$idcredito'";	
...
function abm($cod_ventaFK,$CodUsu,$cod_creditoFK,$monto,$cod_localFK,$totalInteres,$cuota_nro,$motivo,$cod_clienteFK,$diasatraso,$montocuota,$operacion)
{
...
$consulta1="Insert into solicituddescuentointeres (totalSolic, fecha_solic, cod_UsuAprobado, cod_creditoFK, cod_usuarioFK,estado,cod_local,totalInteres,detalle_cuota,estado_uso,motivo,tipo_pago,cod_clienteFK,diasatraso,montocuota,cod_ventaFK)
values('$monto','$fecha_inser','','$cod_creditoFK','$CodUsu','PENDIENTE',$cod_localFK,'$totalInteres','$cuota_nro','PENDIENTE','$motivo','NORMAL','$cod_clienteFK',$diasatraso,'$montocuota','$cod_ventaFK')";
$stmt1 = $mysqli->prepare($consulta1);
...
function buscar($formato="")
{
	...
	$mysqli=conectar_al_servidor();
	 ...
	 $formato=strtolower((string)$formato);
		...
		$sql= "SELECT  idsolicituddescuentointeres,totalSolic,totalInteres,fecha_solic,cod_usuarioFK,cod_creditoFK,estado,cod_UsuAprobado,fecha_apro,motivo,diasatraso,IFNULL(montocuota,0) as montocuota,
		...
		FROM solicituddescuentointeres where estado='PENDIENTE' and totalSolic > 0  ";
...
if (! $stmt->execute()) {
 ...
 if ($valor>0)
 {
	  while ($valor= mysqli_fetch_assoc($result))
	  {
			  ...
			  $filas[]=array(
				  "id_solicitud"=>$idsolicituddescuentointeres,
				  "total_solicitado"=>$total,
				  "total_solicitado_formateado"=>number_format($total,'0',',','.'),
				  "total_interes"=>$totalInteres,
				  "total_interes_formateado"=>number_format($totalInteres,'0',',','.'),
				  "fecha_solicitud"=>$fecha_solic,
				  "usuario"=>$usuario,
				  "detalle_cuota"=>$detalle_cuota,
				  "estado"=>$estado,
				  "local"=>$nombre_local,
				  ...
				  "monto_cuota_formateado"=>number_format($montocuota,'0',',','.')
			  );
			 ...
			 if($formato!="json"){
		 	 $styleName=CargarStyleTable($styleName);
				...
				$pag2.="<div id='divMensajeDescuentoInteres_$idsolicituddescuentointeres'>
				...
				<td  id='td_datos_16' style='display:none'>".$diasatraso."</td>
				<td  id='td_datos_17' style='display:none'>".$detalle_cuota."</td>
				<td  id='td_datos_18' style='display:none'>".number_format($montocuota,'0',',','.')."</td>
				<td style='width:5%'> <span class='status completed' onclick='verCerrarMensajeDescuentoInteresDetalle($idsolicituddescuentointeres)' >VER</span> 				
				</td>
...
function buscarSoliDescuento($fecha1,$fecha2,$UsuSoli,$UsuApro,$producto,$formato="")
{
	$mysqli=conectar_al_servidor();
	 $pagina='';
	 $filas=array();
...
function buscarInforme($fecha1,$fecha2,$usuario,$motivo,$cod_credito,$estado_uso,$estado,$cliente,$formato="")
{
	$mysqli=conectar_al_servidor();
	 $pagina='';
	 $filas=array();
...
$condicionestado_uso="";
if($estado_uso!=""){
	$condicionestado_uso=" and estado_uso = '$estado_uso' ";
}
...
if($motivo!=""){
	$condicionmotivo=" and motivo like '%".$motivo."%'";
		...
		$sql= "SELECT  motivo,cod_creditoFK,detalle_cuota,diasatraso,cod_local,montocuota,estado_uso,
		(select nombre_persona from persona where cod_persona= cod_usuarioFK ) as usuariosoli,
		(select concat(nombre_persona,' ',apellido_persona) from persona where cod_persona= cod_clienteFK ) as cliente
		,(select nombre_persona from persona where cod_persona= cod_UsuAprobado ) as usuarioapro,
		(select Nombre from local where cod_local = sci.cod_local) as local,tipo_pago
		,idsolicituddescuentointeres, estado, fecha_solic, cod_UsuAprobado, totalSolic, ifnull(totalInteres,0) as totalInteres, IFNULL(totalaprobado,0) as totalaprobado FROM solicituddescuentointeres sci where 
		estado !='' ".$condicionFecha.$condicionusuario.$condicioncod_credito.$condicionmotivo.$condicionestado.$condicionestado_uso.$condicioncliente." ";
...
if ( ! $stmt->execute()) {
 ...
 if ($valor>0)
 {
	  while ($valor= mysqli_fetch_assoc($result))
	  {
			  ...
			  $montocuota = ($montocuota != '')? number_format($montocuota,'0',',','.') : $montocuota;
			  
			  
			  $onclick='obtenerdatosMensajeDetalleDescuentoInteres(this)';
			  if($estado_uso == 'FINALIZADO'){
				  $onclick='mensajeDescuentoInteresFinalizado()';
				  $estado_uso ='APLICADO';
			  }
			  ...
			  if($estado =='PENDIENTE'){
				  $styleEstado = 'background-color:yellow;color:#9E9E9E';
			  }
			  
			  if($estado =='RECHAZADO'){
				  $styleEstado = 'background-color:red;color:white';
			  }
			  
			   if($estado == 'APROBADO' && $estado_uso =='APLICADO'){
				  $onclick='mensajeDescuentoInteresFinalizado()';
				  $styleEstado = 'background-color:gray;color:white';
			  }
			...
			$totalDescuento += $totalaprobado;
			  ...
			  $filas[]=array(
				  ...
				  "detalle_cuota"=>$detalle_cuota,
				  "dias_atraso"=>$diasatraso,
				  "monto_cuota_formateado"=>$montocuota,
				  "total_solicitado_formateado"=>number_format($totalSolic,'0',',','.'),
				  "total_aprobado_formateado"=>number_format($totalaprobado,'0',',','.'),
				  "total_interes_formateado"=>number_format($totalInteres,'0',',','.'),
				  "usuario_solicitud"=>$usuariosoli,
				  "fecha_solicitud"=>$fecha_solic,
				  "estado"=>$estado,
				  ...
				  "tipo_pago"=>$tipo_pago,
				  "local"=>$local,
				  "accion"=>$onclick==='obtenerdatosMensajeDetalleDescuentoInteres(this)' ? "editar" : "finalizado",
				  "estilo_estado"=>$styleEstado
			  );
			 ...
			 if($formato!="json"){
		 	 $styleName=CargarStyleTable($styleName);
		  	  $pagina.="
				<table class='$styleName' border='1' cellspacing='1' cellpadding='5'>
				<tr id='tbSelecRegistro' style='$styleEstado' onclick='".$onclick."'>
				...
				<td  id='td_datos_16' style='width:5%'>".$diasatraso."</td>
				<td  id='td_datos_18' style='width:5%'>".$montocuota."</td>
				...
				<td  id='td_datos_7' style='width:5%'>".number_format($totalInteres,'0',',','.')."</td>
				<td  id='td_datos_9' style='width:10%'>".$usuariosoli."</td>
				<td  id='' style='width:10%'>".$fecha_solic."</td>
				<td  id='' style='width:5%'>".$estado."</td>
				<td  id='' style='width:5%'>".$usuarioapro."</td>				
				<td  id='' style='width:5%'>".$estado_uso."</td>				
				<td  id='' style='width:5%'>".$tipo_pago."</td>				
				<td  id='td_datos_10' style='display:none'>".$idsolicituddescuentointeres."</td>				
 ...
 mysqli_close($mysqli);
 $salida=$formato==="json" ? $filas : $pagina;
 $informacion =array("1" => "exito","2" => $salida,"3" => $nroRegistro,"4"=> number_format(abs($totalDescuento),'0',',','.'));
echo json_encode($informacion);	
exit;
