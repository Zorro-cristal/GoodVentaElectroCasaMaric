...
buscarDatosVehiculos($vehiculo,$local,$cod_localFKUSer);
...
$condicionestado="";
	if($estado!=""){
		...
		where dv.estado!=''  ".$condicionfecha.$condicionencargado.$condicionvehiculo.$condiciondetalle." order  by  fecha desc , iddetalle_vehiculo desc  ";
 ...
 $nroRegistro= $valor; 
  if ($valor>0) {
	  while ($valor= mysqli_fetch_assoc($result))
	  {
		...
		$filas[]=array(
			"vehiculo"=>$vehiculo,
			"detalle"=>$det_vehiculo,
			"fecha"=>$fecha,
			"precio"=>$precio,
			"precio_formateado"=>number_format($precio,'0',',','.'),
			"encargado"=>$encargado,
			"kilometro_inicio"=>$nro_inicio,
			"kilometro_inicio_formateado"=>number_format($nro_inicio,'0',',','.'),
			"kilometro_fin"=>$nro_fin,
			"kilometro_fin_formateado"=>number_format($nro_fin,'0',',','.'),
			"estado_registro"=>$estado_redistro
		);
...
$informacion =array("1" => "exito","2" => ($formato==='json' ? $filas : $pagina) ,"3"=> number_format($nroRegistro,'0',',','.') ,"4"=> number_format($totalCosto,'0',',','.') );
echo json_encode($informacion);	
exit;
}
...
function buscarabmNotificaciones($cod_notificacion)
{
	...
	$mysqli=conectar_al_servidor();
		...
		from detalle_vehiculo dv  where  dv.iddetalle_vehiculo='".$cod_notificacion."' and (select notificacion from tipo_detalle_vehiculo where idtipo_detalle_vehiculo=cod_tipo_detalle_vehiculoFK)='SI' ";
...
function BuscarNotificaciones()
{
	...
	$filas=array();
	 ...
	 $pagina="";
		...
		from detalle_vehiculo dv  where dv.estado_redistro!='FINALIZADO' and dv.estado='Activo' and (select notificacion from tipo_detalle_vehiculo where idtipo_detalle_vehiculo=cod_tipo_detalle_vehiculoFK)='SI' and 
 ...
 $Porcentaje=0;
  if ($valor>0) {
	  while ($valor= mysqli_fetch_assoc($result))
	  {
			...
			$filas[]=array(
				"codigo"=>$iddetalle_vehiculo,
				"vehiculo"=>$vehiculo,
				"detalle"=>$det_vehiculo,
				"kilometros_restantes"=>$resultado,
				"kilometros_restantes_formateado"=>number_format($resultado,'0',',','.'),
				"urgente"=>($resultado<=0)
			);
...
exit;
 ...
 function pagina_notificacion_stock_minimo()
{
	...
	$mysqli=conectar_al_servidor();
	 ...
	 $pagina="";
		...
		$sql= "SELECT IFNULL((SELECT sum(entero) FROM stock_producto WHERE cod_stocklocalesFK = stk.idstocklocales),0) as stock_actual,IFNULL((SELECT cantidad FROM stockminimo WHERE cod_productoFK = pr.cod_producto and cod_localFK = stk.cod_localfk),0) as stock_minimo FROM stocklocales stk 
		inner join producto pr on stk.cod_productoFK = pr.cod_producto
		";
  ...
  if ($valor>0) {
	  while ($valor= mysqli_fetch_assoc($result))
	  {
		...
		$filas[]=array(
			...
			"estado_registro"=>$estado_redistro,
			"estado"=>$estado,
			"codigo_tipo_detalle"=>$cod_tipo_detalle_vehiculoFK,
			"codigo_encargado"=>$cod_encargadoFK
		);
...
function buscarDatosVehiculos($vehiculo,$local,$cod_localFKUSer)
{
	...
	if($local!=""){
		...
		if($cod_localFKUSer=='1' || $cod_localFKUSer == '8'){
			$condicionLocal = ' and cod_localFK in(1,8)';
		}else{
			$condicionLocal = " and cod_localFK ='$cod_localFKUSer'";
		}
	 }
	...
	if($vehiculo!=""){
		...
		from vehiculo v 
		where v.estado='activo' ".$condicionLocal.$condicionvehiculo." ";
  ...
  if ($valor>0) {
	  while ($valor= mysqli_fetch_assoc($result))
	  {
			...
			$filas[]=array(
				...
				"kilometraje_formateado"=>number_format($kilometro,'0',',','.')
			);

 
				 $pagina.="<div class='sales'  >
				<h3>".$nombre." - ".$chapa."</h3>
                        <div>
...
<div class='info'> 
<input type='button' value='Mantenimiento' class='btn5' id='$cod_vehiculo' data-url='".$nombre." ".$chapa."' data-email='".$cod_vehiculo."' onclick='verCerrarDetalleVehivulos(this)' style='background-color: #4caf50;margin: 2%;'>							
 
<input type='button' value='Kilometraje' class='btn5' id='$cod_vehiculo' data-url='".$nombre." ".$chapa."' data-email='".$cod_vehiculo."' onclick='verCerrarkilometraje(this)'  style='background-color: #dc3545;margin: 2%;'>							
  ...
  if ($valor>0) {
	  while ($valor= mysqli_fetch_assoc($result))
	  {
		    ...
		    $nro=utf8_encode($valor['nro']);
		  	...
		  	$encargado=utf8_encode($valor['encargado']); 
		$filas[]=array(
			"codigo"=>$idkilometraje,
			"kilometraje"=>$nro,
			"kilometraje_formateado"=>number_format($nro,'0',',','.'),
			"fecha"=>$fecha,
			"encargado"=>$encargado,
			"codigo_encargado"=>$cod_encargadoFK,
			"codigo_vehiculo"=>$cod_vehiculoFK
		);
