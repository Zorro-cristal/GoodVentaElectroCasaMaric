...
if($operacion=="nuevo" )
{
	...
	abm($Cantidad,$Motivo,$Cod_usuarioFK,$Cod_productoFK,$cod_localFK,$operacion);
...
$formato=isset($_POST['formato']) ? utf8_decode($_POST['formato']) : '';
	buscar($Fechafijo,$Fecha1,$Fecha2,$Nombre,$Usuario,$Cod_productoFK,$formato);
...
$formato=isset($_POST['formato']) ? utf8_decode($_POST['formato']) : '';
buscarmas($Fechafijo,$Fecha1,$Fecha2,$Nombre,$Usuario,$Cod_productoFK,$registrocargado,$formato);
...
function descontarStock($Cod_productoFK,$Cantidad,$cod_localFK){
	
	$mysqli=conectar_al_servidor();
	
	$consulta1="update  producto set stock_producto=(stock_producto - ? ) where cod_producto=? and cod_localFK = ?";
...
function abm($Cantidad,$Motivo,$Cod_usuarioFK,$Cod_productoFK,$cod_localFK,$operacion)
{
...
$consulta1="Insert into debaja (Cantidad,Motivo,Cod_usuarioFK,Cod_productoFK,cod_localFK)
values(?,?,?,?,?)";
$stmt1 = $mysqli->prepare($consulta1);
$ss='sssss';
$stmt1->bind_param($ss,$Cantidad,$Motivo,$Cod_usuarioFK,$Cod_productoFK,$cod_localFK);
...
exit;
 ...
 mysqli_close($mysqli);
$informacion =array("1" => "exito");
descontarStock($Cod_productoFK,$Cantidad,$cod_localFK);
echo json_encode($informacion);	
exit;
...
function buscar($Fechafijo,$Fecha1,$Fecha2,$Nombre,$Usuario,$Cod_productoFK,$formato='')
{
...
if ( ! $stmt->execute()) {
 ...
 $totalregistro= $valor; 
 mysqli_close($mysqli);
 $informacion =array("1" => "exito","2" => ($formato==='json' ? $filas : $pagina),"3" => $nroRegistro,"99" => $nroRegistro,"100" => $totalregistro);
echo json_encode($informacion);	
exit;
...
function buscarmas($Fechafijo,$Fecha1,$Fecha2,$Nombre,$Usuario,$Cod_productoFK,$registrocargado,$formato='')
{
...
if ( ! $stmt->execute()) {
 ...
 mysqli_close($mysqli);
 $informacion =array("1" => "exito","2" => ($formato==='json' ? $filas : $pagina),"3" => $nroRegistro,"99"=>$nroRegistro);
echo json_encode($informacion);	
exit;
