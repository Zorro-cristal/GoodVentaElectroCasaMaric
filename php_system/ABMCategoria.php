...
if($funt=="buscar_opciones_filtro_productos_vendidos")
{
	$formato=isset($_POST['formato']) ? $_POST['formato'] : '';
	buscar_filtro_categoria($formato);
...
function buscar_filtro_categoria($formato='')
{
	$mysqli=conectar_al_servidor();
	 $pagina="";  
	 $filas=array();
		$sql= "SELECT cod_categoria,descripcion FROM categoria WHERE estado = 'Activo' order by descripcion asc";
...
if ( ! $stmt->execute()) {
 ...
 if ($valor>0)
 {
	  while ($valor= mysqli_fetch_assoc($result))
	  {
		      ...
		      $cod_categoria=utf8_encode($valor['cod_categoria']);
			 ...
			 if($formato !== "json") {
				 $check = "<input type='checkbox' id='$cod_categoria' name='check_filtro_productos_vendidos' onclick='obteneridfiltroproductosvendidos(this)' checked />";
			 ...
			 array_push($arrayCat,$cod_categoria);
	  }
  }
 
  mysqli_close($mysqli);
  $informacion =array("1" => "exito","2" => ($formato === "json" ? $filas : $pagina),"3"=> $totalresouesta,"4" => $arrayCat);
echo json_encode($informacion);	
exit;
}
