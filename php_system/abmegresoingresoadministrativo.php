...
if($operacion=="nuevo" || $operacion=="editar")
{
	...
	$idgastos_administrativo=$_POST['idegresoingreso'];
$idgastos_administrativo = utf8_decode($idgastos_administrativo);
$monto=$_POST['monto'];
...
$cod_motivo_administrativo=$_POST['cod_motivo'];
$cod_motivo_administrativo = utf8_decode($cod_motivo_administrativo);
...
$fechaDeposito = utf8_decode($fechaDeposito);
	...
	abm($fechaDeposito,$cod_motivo_administrativo,$Arreglo,$nroboleta, $banco , $nrocuenta ,$idgastos_administrativo,$monto,$motivo,$fecha,$estado,$personales,$cod_usuario,$cod_local,$tipo,$extgasto,$operacion);
...
$idgastos_administrativo=$_POST['idegresoingresoadministrativo'];
$idgastos_administrativo = utf8_decode($idgastos_administrativo);


$cod_usuario = $user;

	confirmarEgresoIngreso($idgastos_administrativo);
...
$agrupacionformulariogasto=$_POST['agrupacionformularioegresoingreso'];
...
buscar($permisover,$user,$arreglo,$fecha1,$fecha2,$estado,$cod_local,$tipo,$usuario,$fecha,$motivo,$confirmado,$agrupacionformulariogasto,$banco,$nroboleta,$monto);
...
function abm($fechaDeposito,$cod_motivo_administrativo,$Arreglo,$nroboleta, $banco , $nrocuenta,$idgastos_administrativo,$monto,$motivo,$fecha,$estado,$personales,$cod_usuario,$cod_local,$tipo,$extgasto,$operacion)
{
...
$consulta1="Insert into gastos_administrativo (arreglo,monto,motivo,fecha,estado,cod_usuario,personales,cod_descripcion_local_gasto_administrativoFK,tipo,nroboleta,banco,nrocuenta,cod_motivo_administrativo,fechaDeposito)
values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
$stmt1 = $mysqli->prepare($consulta1);
$ss='ssssssssssssss';
$stmt1->bind_param($ss,$Arreglo,$monto,$motivo,$fecha,$estado,$cod_usuario,$personales,$cod_local,$tipo,$nroboleta, $banco , $nrocuenta,$cod_motivo_administrativo,$fechaDeposito);
}
 
if($operacion=="editar")
{

$consulta1="Update gastos_administrativo set arreglo=?, monto=?,motivo=?,fecha=?,estado=?,
personales=?,cod_descripcion_local_gasto_administrativoFK=?,tipo=?,nroboleta=?,banco=?,nrocuenta=?,cod_motivo_administrativo=?,fechaDeposito='$fechaDeposito' where idgastos_administrativo=?";
$stmt1 = $mysqli->prepare($consulta1);
$ss='sssssssssssss';
$stmt1->bind_param($ss,$Arreglo,$monto,$motivo,$fecha,$estado,$personales,$cod_local,$tipo,$nroboleta,$banco,$nrocuenta,$cod_motivo_administrativo,$idgastos_administrativo); 
...
if($operacion=='editar'){
	cargar_foto_gastos($idgastos_administrativo);
}else{
...
$foto = base64_decode($foto);
		     ...
		     $donde="../fotos/gastos_administrativo/";

			  $id_foto = generarCodigoAleatorio(7);
                $id_f=subir_imagen_base64($donde,$foto,$id_foto,$ext);
$ruta="/GoodVentaElectroCasaMaric/fotos/gastos_administrativo/".$id_foto.$id_f.'.'.$ext;

$mysqli=conectar_al_servidor();
$consulta="UPDATE gastos_administrativo SET url = '$ruta' WHERE idgastos_administrativo  = '$cod_gasto'";	
...
function confirmarEgresoIngreso($idgastos_administrativo)
{

$mysqli=conectar_al_servidor();


$consulta1="Update gastos_administrativo set confirmado='SI' where idgastos_administrativo='$idgastos_administrativo'";
$stmt1 = $mysqli->prepare($consulta1);
...
function buscar($permisover,$user,$arreglo,$fecha1,$fecha2,$estado,$cod_local,$tipo,$usuario,$fecha,$cod_motivo_administrativo,$confirmado,$agrupacionformulariogasto,$banco,$nroboleta,$monto)
{
	$mysqli=conectar_al_servidor();
	 $pagina='';
	 $filas=array();
	 $formato=isset($_POST['formato']) ? utf8_decode($_POST['formato']) : '';
	 $condicionCodLocal=" and g.cod_descripcion_local_gasto_administrativoFK='$cod_local' ";
		 if($cod_local==""){
			$condicionCodLocal=" "; 
		 }
		 $condiciontipo="";
		 ...
		 $condicionmotivo="";
		 if($cod_motivo_administrativo!=""){
			$condicionmotivo=" and  cod_motivo_administrativo = '".$cod_motivo_administrativo."' "; 
		 }
		 
		 $condicionagrupacion = '';
		 if($agrupacionformulariogasto == '2'){
			 $condicionagrupacion = ' group by cod_motivo_administrativo';
		 }
		...
		$sql= "Select fechaDeposito,arreglo,monto, ifnull((select descripcion from  motivo_e_i_administrativo where idmotivo_e_i_administrativo=cod_motivo_administrativo),'') as descripcion,motivo,fecha,estado,cod_usuario,idgastos_administrativo,tipo,cod_descripcion_local_gasto_administrativoFK,nroboleta,banco,nrocuenta,cod_motivo_administrativo,
		...
		(Select descripcion from descripcion_local_gasto_administrativo where cod_descripcion_local_gasto_administrativoFK=cod_descripcion_local_gasto_administrativo) as nombrelocal
		from gastos_administrativo g where  estado='$estado' ".$condicionpermisover.$condicionCodLocal.$condicionarreglo.$condiciontipo.$condicionfecha.$condicionusuario.$condicionrangofechas.$condicionmotivo.$condicionconfirmado.$condicionagrupacion.$condicionbanco.$condicionnroboleta.$condicionmonto;
   ...
   $fechaFiltro = $fecha;
   $cod_motivoFiltro = $cod_motivo_administrativo;
...
if ( ! $stmt->execute()) {
 ...
 if ($valor>0)
 {
	  while ($valor= mysqli_fetch_assoc($result))
	  {
		      ...
		      $cod_motivo_administrativo=$valor['cod_motivo_administrativo'];
		      $descripcion=utf8_encode($valor['descripcion']);
		      $idgastos_administrativo=$valor['idgastos_administrativo'];
		  	  ...
		  	  $cod_local=utf8_encode($valor['cod_descripcion_local_gasto_administrativoFK']);
			  ...
			  $MotivoGrupo="";
			  if($agrupacionformulariogasto == '2'){
				  $totalMonto = obtener_total_agrupacion_motivo($arregloFiltro,$fecha1Filtro,$fecha2Filtro,$estadoFiltro,$cod_localFiltro,$tipoFiltro,$usuarioFiltro,$fechaFiltro,$cod_motivo_administrativo,$confirmadoFiltro);
			  ...
			  if($url != ''){
				...
				$btnVer = "<input type=\"button\" value=\"Ver\" style=\"width:50px\" class=\"btn4\" onclick=\"verdocumentoClienteSolicitud('$url')\" />";
			  }
...
if($tipo!="Deposito"){
	$fechaDeposito="";
			  ...
			  $filas[]=array(
				  ...
				  "estado" => $estado,
				  "cod_local" => $cod_local,
				  "cod_motivo" => $cod_motivo_administrativo,
				  "url" => $url,
				  "foto_url" => $fotourl,
				  "foto_ext" => $ext,
...
<tr id='tbSelecRegistro' onclick='obtenerdatosabmEgresoIngresoAdministrativo(this)'>
<td id='td_id' style='width:5%; background-color: #efeded;color:red'>".$idgastos_administrativo."</td>
...
<td  					style='display:none'>".$cod_motivo_administrativo."</td>
...
$informacion =array("1" => "exito","2" => $formato=='json' ? $filas : $pagina,"3" => $nroRegistro,"4" =>  number_format($agrupacionformulariogasto == '2' ? $totalMontoAgrupado : $totalGasto,'0',',','.'));
echo json_encode($informacion);	
exit;
...
$condicionCodLocal=" and g.cod_descripcion_local_gasto_administrativoFK='$cod_local' ";
		 ...
		 $condicionmotivo="";
		 if($motivo!=""){
			$condicionmotivo=" and  cod_motivo_administrativo = '".$motivo."' "; 
		 }
		 
		$sql= "Select arreglo,monto, ifnull((select descripcion from  motivo_e_i_administrativo where idmotivo_e_i_administrativo=cod_motivo_administrativo),'') as descripcion,motivo,fecha,estado,cod_usuario,idgastos_administrativo,tipo,cod_descripcion_local_gasto_administrativoFK,nroboleta,banco,nrocuenta,cod_motivo_administrativo,
		(Select nombre_persona from persona where cod_persona=cod_usuario) as usuarionombre,confirmado,
		(Select descripcion from descripcion_local_gasto_administrativo where cod_descripcion_local_gasto_administrativoFK=cod_descripcion_local_gasto_administrativo) as nombrelocal
		from gastos_administrativo g where  estado='$estado' ".$condicionCodLocal.$condicionarreglo.$condiciontipo.$condicionfecha.$condicionusuario.$condicionrangofechas.$condicionmotivo.$condicionconfirmado;
...
function buscarevaluacionGasto($fecha1,$fecha2,$cod_local,$tipo_arreglo)
{
	$mysqli=conectar_al_servidor();
	 ...
	 $condicionCodLocal=" and g.cod_local='$cod_local' ";
		...
		$sql= "Select monto,motivo,fecha,estado,cod_usuario,idgastos_administrativo,personales,cod_local,
		...
		from gastos_administrativo g where fecha>='$fecha1' and fecha<='$fecha2' and estado='Activo' ".$condicionCodLocal.$condiciontipoarreglo;
...
function buscaroption()
{
	...
	$filas=array();
		...
		$sql= "Select * from motivo_e_i_administrativo where estado='Activo' order by descripcion asc  ";
...
if ( ! $stmt->execute()) {
 ...
 if ($valor>0)
 {
	  while ($valor= mysqli_fetch_assoc($result))
	  {
		      ...
		      $idmotivo_e_i=$valor['idmotivo_e_i_administrativo'];
		  	  $descripcion=utf8_encode($valor['descripcion']);
			  $filas[]=array(
				  "codigo" => $idmotivo_e_i,
				  "descripcion" => $descripcion,
...
$consulta1="Insert into motivo_e_i_administrativo (descripcion,estado) values (upper(?),'$estado')";
$stmt1 = $mysqli->prepare($consulta1);
$ss='s';
...
$consulta1="update motivo_e_i_administrativo SET descripcion = upper('$motivo'), estado ='$estado' WHERE idmotivo_e_i_administrativo ='$idabm'";
$stmt1 = $mysqli->prepare($consulta1);
...
function buscarabmmotivoingresoegreso($buscar,$Estado)
{
	$mysqli=conectar_al_servidor();
	 ...
	 $filas=array();
	 $formato=isset($_POST['formato']) ? $_POST['formato'] : '';
		$sql= "Select idmotivo_e_i_administrativo,descripcion,Estado
        from motivo_e_i_administrativo where descripcion like ?  and Estado=? order by descripcion asc ";
...
if ( ! $stmt->execute()) {
 ...
 if ($valor>0)
 {
	  while ($valor= mysqli_fetch_assoc($result))
	  {
			  ...
			  $pagina.="
			  <table class='$styleName' border='1' cellspacing='1' cellpadding='5'>
			  <tr id='tbSelecRegistro' onclick='ObtenerdatosAbmMotivoEgresoIngresoAdministrativo(this)'>
