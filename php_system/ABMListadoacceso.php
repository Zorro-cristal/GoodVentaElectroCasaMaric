...
if($funt=="eliminarAcceso" )
{
	
	
	$idlistadodeacceso=$_POST['idlistadoacceso'];
    $idlistadodeacceso = utf8_decode($idlistadodeacceso);
	eliminarAcceso($idlistadodeacceso);

}

if($funt=="obtener_permisos" )
{
	
	
	
	obtener_permisos($user);
...
$mysqli=conectar_al_servidor(); 
				...
				$consulta= "Select count(*) from listadodeacceso where codigo=?  ";
...
function eliminarAcceso($idlistadodeacceso)
{
	
	if($idlistadodeacceso=="" ){
$informacion =array("1" => "DI");
echo json_encode($informacion);	
exit;
	}
    ...
    $consulta="delete from detallesniveles where idlistadodeacceso='$idlistadodeacceso'";	
	$stmt = $mysqli->prepare($consulta);
...
$consulta="delete from accesosuser where idlistadodeaccesoFK='$idlistadodeacceso'";	
	$stmt = $mysqli->prepare($consulta);
...
$consulta="delete from listadodeacceso where idlistadodeacceso='$idlistadodeacceso'";	
	$stmt = $mysqli->prepare($consulta);
...
$controltitulo="";
 ...
 if ($valor>0)
 {
	  while ($valor= mysqli_fetch_assoc($result))
	  {
			  ...
			  $filas[]=array(
				"idlistadodeacceso" => $idlistadodeacceso,
				"nro" => $nro,
		  	...
		  	$ordenInput="<input onkeyup='if(event.keyCode === 13){guardarordenlistadoacceso(this)}'  value='$orden' style='width: 80px;text-align: center;' type='text'  class='input3' id='$idlistadodeacceso'  />";
			  ...
			  <td id='td_id' style='display:none;'>".$idlistadodeacceso."</td>
			   <td  style='width:10%' >".$ordenInput."</td>
...
function obtener_permisos($buscar)
{
	$mysqli=conectar_al_servidor();
	 $datos[0]="";
		...
		where usuarios_idusario = '$buscar' and acus.tipo='Administrativo' order by lta.codigo asc";
