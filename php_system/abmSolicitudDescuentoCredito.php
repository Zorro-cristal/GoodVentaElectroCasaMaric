...
if($operacion=="nuevo" )
{
	...
	$monto=$_POST['totalSolic'];
	$monto = quitarseparadormiles($monto);
	$montocuota=$_POST['cuota_monto'];
	...
	abm($CodUsu,$cod_creditoFK,$monto,$cod_localFK,$detalle_cuota,$motivo,$cod_clienteFK,$montocuota,$tipo);
...
if($operacion=="buscarInforme")
{
	...
	buscarInforme($fecha1,$fecha2,$usuario,$motivo,$cod_credito,$estado,$cliente,$formato);
...
if($operacion=="buscarHistorial")
{
	...
	buscarHistorial($cod_credito,$formato);
...
$monto_descuento = total_sumatoria_descuento_credito_solicitado($cod_credito);
...
function abm($CodUsu,$cod_creditoFK,$monto,$cod_localFK,$detalle_cuota,$motivo,$cod_clienteFK,$montocuota,$tipo)
{
...
values('$monto','$fecha_inser','$cod_creditoFK','$CodUsu','PENDIENTE',$cod_localFK,'$detalle_cuota','$motivo','$cod_clienteFK','$montocuota','$tipo')";
...
function buscar($formato="")
{
	...
	$mysqli=conectar_al_servidor();
	 ...
	 $formato=strtolower((string)$formato);
		...
		$sql= "SELECT  idsolicituddescuentocredito,abs(totalSolic) as totalSolic,fecha_solic,cod_usuarioFK,cod_creditoFK,estado,cod_UsuAprobado,fecha_apro,motivo,IFNULL(monto_cuota,0) as monto_cuota,tipo_solicitud,
		...
		FROM solicituddescuentocredito where estado='PENDIENTE' and abs(totalSolic) > 0  ";
...
if (! $stmt->execute()) {
 ...
 if ($valor>0)
 {
	  while ($valor= mysqli_fetch_assoc($result))
	  {
			  ...
			  $tipo_solicitud=utf8_encode($valor['tipo_solicitud']);
			  // $diasatraso=utf8_encode($valor['diasatraso']);

			  $filas[]=array(
				  "id_solicitud"=>$idsolicituddescuentocredito,
				  "total_solicitado"=>$total,
				  "total_solicitado_formateado"=>number_format($total,'0',',','.'),
				  "monto_cuota"=>$monto_cuota,
				  "monto_cuota_formateado"=>number_format($monto_cuota,'0',',','.'),
			 ...
			 if($formato!="json"){
		 	 $styleName=CargarStyleTable($styleName);
				...
				<td  id='td_datos_19' style='display:none'>".$tipo_solicitud."</td>
...
function buscarInforme($fecha1,$fecha2,$usuario,$motivo,$cod_credito,$estado,$cliente,$formato="")
{
	$mysqli=conectar_al_servidor();
	 $pagina='';
	 $filas=array();
...
if($motivo!=""){
	$condicionmotivo=" and motivo like '%".$motivo."%'";
		...
		estado !=''  ".$condicionFecha.$condicionusuario.$condicioncod_credito.$condicionmotivo.$condicionestado.$condicioncliente." ";
...
if ( ! $stmt->execute()) {
 ...
 if ($valor>0)
 {
	  while ($valor= mysqli_fetch_assoc($result))
	  {
			  ...
			  $styleEstadoUso = 'background-color:#dfdf39;color:white';
			...
			$totalDescuento += $totalaprobado;
			  ...
			  $filas[]=array(
				  ...
				  "cliente"=>$cliente,
				  "detalle_cuota"=>$detalle_cuota,
				  "monto_cuota_formateado"=>$monto_cuota,
				  ...
				  "usuario_aprobado"=>$usuarioapro,
				  "local"=>$local,
				  "accion"=>$onclick==='obtenerdatosMensajeDetalleDescuentoCredito(this)' ? "editar" : "finalizado",
				  "estilo_estado"=>$styleEstadoUso
			  );
...
function buscarHistorial($cod_credito,$formato="")
{
	$mysqli=conectar_al_servidor();
	 $pagina='';
	 $filas=array();
...
if($cod_credito!=""){
	$condicioncod_credito=" and cod_creditoFK = '$cod_credito' ";
		...
		(select concat(nombre_persona,' ',apellido_persona) from persona where cod_persona= cod_clienteFK ) as cliente
		,(select nombre_persona from persona where cod_persona= cod_UsuAprobado ) as usuarioapro,
		tipo_solicitud,fecha_apro,
		(select Nombre from local where cod_local = sci.cod_local) as local
		,idsolicituddescuentocredito, estado, fecha_solic, cod_UsuAprobado, totalSolic, IFNULL(totalaprobado,0) as totalaprobado FROM solicituddescuentocredito sci where 
		estado !=''  ".$condicioncod_credito;
...
if ( ! $stmt->execute()) {
 ...
 if ($valor>0)
 {
	  while ($valor= mysqli_fetch_assoc($result))
	  {
			  ...
			  $monto_cuota=utf8_encode($valor['monto_cuota']);
			  $tipo_solicitud=utf8_encode($valor['tipo_solicitud']);
			  $fecha_apro=utf8_encode($valor['fecha_apro']);
			...
			$totalDescuento += $totalaprobado;
			 ...
			 if($formato!="json"){
		 	 $styleName=CargarStyleTable($styleName);
				...
				<td  id='td_datos_6' style='width:10%'>".number_format($totalSolic,'0',',','.')."</td>
				<td  id='' style='width:10%'>".$fecha_solic."</td>
				<td  id='td_datos_14' style='width:10%'>".$motivo."</td>
				<td  id='' style='width:10%'>".$tipo_solicitud."</td>
				<td  id='td_datos_9' style='width:10%'>".$usuariosoli."</td>
				<td  id='' style='width:10%'>".$estado."</td>
				<td  id='' style='width:10%'>".$fecha_apro."</td>
				</tr>
				</table>";
			 }
...
function comprobarSolicitudDescuentoInteresTipoParcial($cod_clienteFK)
{
	$mysqli=conectar_al_servidor();
	
		$sql= "Select * from solicituddescuentointeres where cod_clienteFK = '$cod_clienteFK' and estado_uso = 'PENDIENTE' and tipo_pago = 'PARCIAL'";
...
function comprobarEstadosSolicitudDescuentoInteres($cod_clienteFK)
{
	$mysqli=conectar_al_servidor();
	
		$sql= "Select * from solicituddescuentointeres where cod_clienteFK = '$cod_clienteFK' and estado_uso = 'PENDIENTE' and (tipo_pago = 'NORMAL' || tipo_pago = 'PARCIAL')";
