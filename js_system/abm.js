...
function abmlistado(cant, fecha, estado, cod_producto, cod_cobrador, cantvendido, cod_local, idlistado, accion) {
	...
	var OpAjax = $.ajax({
		...
		data: datos,
		url: "/GoodVentaElectroCasaMaric/php_system/abmlistado.php",
		type: "post",
		cache: false,
		contentType: false,
		processData: false,
...
function MasFiltrosRefinaciar(datos){
	if(document.getElementById("divMasFiltrosRefinaciar").style.display==""){
		...
		datos.src="/GoodVentaElectroCasaMaric/iconos/filtros.png";
	}else{
		$("div[id=divMasFiltrosRefinaciar]").slideDown(500);
		datos.src="/GoodVentaElectroCasaMaric/iconos/filtros2.png";
	}
}
...
function crearCeldaCreditoRefin(fila, valor, id, ancho, oculto){
...
function renderCreditosRefin(registros, destino){
	...
	(registros || []).forEach(function(registro){
		...
		if(registro.estado_visual === "pagado"){
			fila.style.backgroundColor = "#ccc";
			fila.style.color = "#000";
		}else if(registro.estado_visual === "cancelado"){
			fila.style.textDecoration = "line-through";
		}
		fila.onclick = function(){
			if(registro.accion === "editar"){ obtenerDatosCreditosRefinEditar(this); }
			else if(registro.accion === "refinanciar"){ obtenerDatosCreditosRefinanciacion(this); }
			else if(registro.accion === "pagos"){ obtenerPagosCreditosRefinanciacion(this); }
		};
		crearCeldaCreditoRefin(fila, registro.id_credito, "td_datos_1", "", true);
		crearCeldaCreditoRefin(fila, registro.plazo, "td_datos_2", "10%", false);
		crearCeldaCreditoRefin(fila, registro.fecha_pago, "td_datos_3", "10%", false);
		crearCeldaCreditoRefin(fila, registro.monto, "td_datos_5", "10%", false);
		crearCeldaCreditoRefin(fila, registro.id_venta, "td_datos_4", "", true);
		crearCeldaCreditoRefin(fila, registro.total_pago, "td_datos_10", "", true);
		crearCeldaCreditoRefin(fila, registro.total_interes, "td_datos_11", "10%", false);
		crearCeldaCreditoRefin(fila, registro.descuento, "td_datos_12", "10%", false);
		crearCeldaCreditoRefin(fila, registro.total, "", "10%", false);
		crearCeldaCreditoRefin(fila, registro.total_pago, "", "10%", false);
		crearCeldaCreditoRefin(fila, registro.deuda_actual, "td_datos_6", "10%", false);
		crearCeldaCreditoRefin(fila, registro.estado, "td_datos_7", "", true);
		crearCeldaCreditoRefin(fila, registro.numero_recibo, "td_datos_8", "", true);
		crearCeldaCreditoRefin(fila, registro.dias_atraso, "td_datos_9", "", true);
		crearCeldaCreditoRefin(fila, registro.tipo_pago, "td_datos_13", "", true);
		crearCeldaCreditoRefin(fila, registro.dias_gracia, "td_datos_15", "", true);
		crearCeldaCreditoRefin(fila, registro.interes, "td_datos_14", "", true);
     ...
     // console.log(Respuesta)
		 ...
		 if (Respuesta=="UI")
		  ...
		  var datos_buscados=datos[2];
			...
			renderCreditosRefin(datos_buscados, document.getElementById("table_abm_opciones_RefinVenta"))
...
document.getElementById("lblReciboNumeroLegal1").textContent = NroFacturaLegal || "";
document.getElementById("lblReciboNumeroLegal2").textContent = NroFacturaLegal || "";

 
 
	document.getElementById("lblReciboContado1").innerHTML = TipoFactura
	document.getElementById("lblReciboContado2").innerHTML =  TipoFactura
...
document.getElementById("lblReciboFechaEmision1").innerHTML = document.getElementById('inptFechaVenta').value;
document.getElementById("lblReciboFechaEmision2").innerHTML =  document.getElementById('inptFechaVenta').value;
...
var documento= document.getElementById("divImpresionRecibo").innerHTML;
     localStorage.setItem("reporte", documento);
	  ...
	  // if(cod_localFKUSer=="1"){
		var URL= "/GoodVentaElectroCasaMaric/system/reportFacturas.html"
        window.open(URL, 'Imprimir', 'toolbar=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=1,left = 0');
	  // }
		 // if(cod_localFKUSer=="2"){
		 // var URL= "/GoodVentaElectroCasaMaric/system/reportFacturasSucUno.html"
        // window.open(URL, 'Imprimir', 'toolbar=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=1,left = 0');
		 // }
...
var fecha = new Date(vencimientopagare);
	...
	+".pagare-maric{box-sizing:border-box;width:100%;max-width:277mm;margin:0 auto;padding:3mm 5mm;color:#111;font-family:Arial,Helvetica,sans-serif;font-size:11px;line-height:1.18;}"
	+".pagare-maric *{box-sizing:border-box;}"
	+".pagare-encabezado{display:flex;align-items:flex-start;gap:8mm;}"
	+".pagare-marca{width:34mm;text-align:center;font-weight:900;line-height:.84;font-size:22px;padding-top:2mm;}"
	+".pagare-marca small{display:block;font-size:7px;line-height:1.2;margin-top:2mm;font-weight:600;}"
	+".pagare-empresa{flex:1;text-align:center;}"
	+".pagare-empresa h2{margin:0;font-size:24px;line-height:1;}"
	+".pagare-empresa h3{margin:1mm 0 0;font-size:9px;}"
	+".pagare-empresa p{margin:1mm 0 0;font-size:7px;line-height:1.25;}"
	+".pagare-datos{width:64mm;border-collapse:separate;border-spacing:2mm 1mm;font-weight:bold;}"
	+".pagare-datos td:first-child{white-space:nowrap;text-align:right;}"
	+".pagare-casilla{height:9mm;border:1px solid #444;border-radius:2mm;background:#fff;padding:2mm 3mm;font-size:12px;font-weight:normal;}"
	+".pagare-titulo{font-size:19px;margin:3mm 0 2mm 27mm;}"
	+".pagare-fecha{text-align:right;font-size:13px;margin:0 9mm 2mm 0;}"
	+".pagare-linea{margin:1.5mm 0;font-size:13px;}"
	+".pagare-renglon{display:block;min-height:8mm;border:1px solid #555;border-radius:4mm;margin:2mm 0;padding:2mm 4mm;font-weight:bold;text-transform:uppercase;}"
	+".pagare-recibido{font-size:13px;margin-top:3mm;}"
	+".pagare-legal{font-size:10px;line-height:1.12;text-align:justify;margin:3mm 0;}"
	+".pagare-firmas{width:100%;border-collapse:collapse;margin-top:5mm;font-size:13px;}"
	+".pagare-firmas td{width:50%;vertical-align:top;padding-right:13mm;}"
	+".pagare-firmas td+td{padding-right:0;padding-left:13mm;}"
	+".pagare-firma-linea{display:grid;grid-template-columns:max-content 1fr;gap:3mm;margin:3mm 0;align-items:end;}"
	+".pagare-puntos{border-bottom:1px dotted #222;min-height:5mm;padding:0 2mm;}"
	+"@media print{.pagare-maric{padding:0;}body{margin:0;} }"
	+"</style>"
...
document.getElementById("divpagare").innerHTML=pagina;
 var documento= document.getElementById("DivImprimirPagares").innerHTML;
     localStorage.setItem("reporte", documento);
	  ...
	  var URL= "/GoodVentaElectroCasaMaric/system/report.html"
        window.open(URL, 'Imprimir', 'toolbar=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=1,left = 0');
}

function escaparHtmlPagare(valor){
...
function calcularIntereseEnCuota() {
	...
	var deuda=QuitarSeparadorMilValor(totalPagadoCuota)
	var i = parseFloat(i);
	var t = parseFloat(t);
	var deuda = parseFloat(deuda);
	document.getElementById('inptMontoAPagar').value =  i+deuda;
...
function ImprimirExtracto(ZonaCliente,DireccionCliente,DocumentoCliente,GaranteCliente,TelefonGarante,CobradorCliente,telefonoCliente) {
...
+"<p class='pTituloC' >"+document.getElementById("inptBuscarInfVentasCompletadasF1").value+"</p>"
+"</td>"

+"<td style='width:20%;text-align:left'>"
+"<p class='pTituloC'><b>FECHA FIN</b></p>"
+"<p class='pTituloC' >"+document.getElementById("inptBuscarInfVentasCompletadasF2").value+"</p>"
+"</td>"

+"<td style='width:20%;text-align:left'>"
+"<p class='pTituloC'><b>DIAS ATRASO</b></p>"
+"<p class='pTituloC' >"+document.getElementById("inptBuscarInfVentasCompletadas6").value+"</p>"
+"</td>"
...
+"</td>"
+"</tr>"
+"</table><br><br><center><h1 class='pTituloD' >LISTA DE CUENTAS COMPLETADAS</h1><br></center>"
document.getElementById("divCabeceraImpresiones").innerHTML=pagina
document.getElementById("tbTitulosImpresiones").innerHTML=document.getElementById("tdTituloImpreVentasCompletadas").innerHTML
document.getElementById("tbDatosImpresiones").innerHTML=document.getElementById("table_historial_Ventas_Completadas").innerHTML
}
 
if (ventana == "ListaCliente") {
...
+"<p class='pTituloC' >"+$("select[id=inptBuscarAbmCliente8]").children(":selected").text()+"</p>"
+"</td>"
+"<td style='width:20%;text-align:left'>"
+"<p class='pTituloC'><b>Fecha de impresión</b></p>"
+"<p class='pTituloC' >"+fechaimpresion+"</p>"
+"</td>"
+"</tr>"
+"</table><br><br><center><h1 class='pTituloD' >LISTA DE CLIENTES</h1><br></center>"
document.getElementById("divCabeceraImpresiones").innerHTML=pagina
document.getElementById("tbTitulosImpresiones").innerHTML=document.getElementById("tbListadeClientes").innerHTML
document.getElementById("tbDatosImpresiones").innerHTML=document.getElementById("table_abm_clientes").innerHTML
}

if (ventana == "productonovendidos") {
	 
		pagina =
"<table class='TableRepor0' style='width:100%'>"
+"<tr>"
+"<td style='width:20%;text-align:left'>"
+"<p class='pTituloC'><b>AÑOS</b></p>"
+"<p class='pTituloC' >"+$("select[id=inptRangoProductosNoVendidos]").children(":selected").text()+"</p>"
+"</td>"
+"<td style='width:20%;text-align:left'>"
+"<p class='pTituloC'><b>MARCA</b></p>"
+"<p class='pTituloC' >"+$("select[id=inptMarcaInformeProductosNoVendidos]").children(":selected").text()+"</p>"
+"</td>"

+"<td style='width:20%;text-align:left'>"
+"<p class='pTituloC'><b>CATEGORIA</b></p>"
+"<p class='pTituloC' >"+$("select[id=inptCategoriaProductoInformeProductosNoVendidos]").children(":selected").text()+"</p>"
+"</td>"
...
+"</td>"
+"</tr>"
+"</table><br><br><center><h1 class='pTituloD' >PRODUCTOS NO VENDIDOS</h1><br></center>"
document.getElementById("divCabeceraImpresiones").innerHTML=pagina
document.getElementById("tbTitulosImpresiones").innerHTML=document.getElementById("tdTituloImpreNoVendidos").innerHTML
document.getElementById("tbDatosImpresiones").innerHTML=document.getElementById("table_comision_productosNoVendidos").innerHTML
}
 
if (ventana == "ListaClienteTareaCobrador") {

		pagina =
"<table class='TableRepor0' style='width:100%'>"
+"<tr>"
+"<td style='width:25%;text-align:left'>"
+"<p class='pTituloC'><b>FECHAS:</b></p>"
+"<p class='pTituloC' >"+fechasTarea+"</p>"
+"</td>"
+"<td style='width:25%;text-align:left'>"
+"<p class='pTituloC'><b>COBRADOR:</b></p>"
+"<p class='pTituloC' >"+CobradorTarea+"</p>"
+"</td>"
+"<td style='width:25%;text-align:left'>"
+"<p class='pTituloC'><b>DESCRIPCION:</b> </p>"
+"<p class='pTituloC' >"+descripcionTarea+"</p>"
+"</td>"
+"<td style='width:25%;text-align:left'>"
+"<p class='pTituloC'><b>Fecha de impresión</b></p>"
+"<p class='pTituloC' >"+fechaimpresion+"</p>"
+"</td>"
+"</tr>"
+"</table><br><br><center><h1 class='pTituloD' >LISTADO DE CLIENTES</h1><br></center>"
document.getElementById("divCabeceraImpresiones").innerHTML=pagina
document.getElementById("tbTitulosImpresiones").innerHTML=document.getElementById("tdTituloListaClienteTareaCobrador").innerHTML
document.getElementById("tbDatosImpresiones").innerHTML=document.getElementById("table_ListaClienteTareaCobrador").innerHTML
}
 
if (ventana == "MantenimientoVehivulos") {
	
		pagina =
"<table class='TableRepor0' style='width:100%'>"
+"<tr>"
+"<td style='width:20%;text-align:left'>"
+"<p class='pTituloC'><b>Desde:</b></p>"
+"<p class='pTituloC' >"+document.getElementById("inptbuscarInformeMantenimientoVehivulosF1").value+"</p>"
+"</td>"
+"<td style='width:20%;text-align:left'>"
+"<p class='pTituloC'><b>Hasta:</b></p>"
+"<p class='pTituloC' >"+document.getElementById("inptbuscarInformeMantenimientoVehivulosF2").value+"</p>"
+"</td>"
+"<td style='width:20%;text-align:left'>"
+"<p class='pTituloC'><b>Encargado:</b></p>"
+"<p class='pTituloC' >"+$("select[id=inptBuscarInfMantenimientoVehivulos3]").children(":selected").text()+"</p>"
+"</td>"
+"<td style='width:20%;text-align:left'>"
+"<p class='pTituloC'><b>Vehiculos:</b></p>"
+"<p class='pTituloC' >"+document.getElementById("inptBuscarMantenimientoVehivulos1").value+"</p>"
+"</td>"
+"<td style='width:20%;text-align:left'>"
+"<p class='pTituloC'><b>Fecha de impresión</b></p>"
+"<p class='pTituloC' >"+fechaimpresion+"</p>"
+"</td>"
+"</tr>"
+"</table><br><br><center><h1 class='pTituloD' >MANTENIMIENTO VEHICULOS</h1><br></center>"
...
+"<p class='pTituloC' >"+ document.getElementById("inptTotalRegistoInformeMantenimientoVehivulos").value+"</p>"
+"</td>"
+"<td style='width:20%;text-align:left'>"
+"<p class='pTituloC'><b>Total  Costo:</b></p>"
+"<p class='pTituloC' >"+document.getElementById("inptTotalCostoInformeMantenimientoVehivulos").value+"</p>"
+"</td>"
...
document.getElementById("tbTitulosImpresiones").innerHTML=document.getElementById("tdTituloMantenimientoVehivulos").innerHTML
document.getElementById("tbDatosImpresiones").innerHTML=document.getElementById("table_informe_MantenimientoVehivulos").innerHTML
}
 
if (ventana == "carteracliente") {
...
+"<td style='width:20%;text-align:left'>"
+"<p class='pTituloC'><b>Desde</b></p>"
+"<p class='pTituloC' >"+document.getElementById("inptBuscarInformeCreditoF1informe").value+"</p>"
+"</td>"
+"<td style='width:20%;text-align:left'>"
+"<p class='pTituloC'><b>Hasta</b></p>"
+"<p class='pTituloC' >"+document.getElementById("inptBuscarInformeCreditoF2informe").value+"</p>"
+"</td>"


+"<td style='width:20%;text-align:left'>"
+"<p class='pTituloC'><b>Total Cliente</b></p>"
+"<p class='pTituloC' >"+document.getElementById("inptRegistroNroHistorialInformeCredito").value+"</p>"
+"</td>"

+"<td style='width:20%;text-align:left'>"
+"<p class='pTituloC'><b>Total a Cobrar</b></p>"
+"<p class='pTituloC' >"+document.getElementById("inptRegistroTotalNetoInformeCredito").value+"</p>"
+"</td>"
+"<td style='width:20%;text-align:left'>"
+"<p class='pTituloC'><b>Local</b></p>"
+"<p class='pTituloC' >"+$("select[id=inptlocalInformeCredito]").children(":selected").text()+"</p>"
+"</td>"
+"<td style='width:20%;text-align:left'>"
+"<p class='pTituloC'><b>Fecha de impresión</b></p>"
+"<p class='pTituloC' >"+fechaimpresion+"</p>"
+"</td>"
+"</tr>"
+"</table><br><br><center><h1 class='pTituloD' >CARTERA DE CLIENTES</h1><br></center>"
...
+"<tr>"
+"<td style='width:20%;text-align:left'>"
+"<p class='pTituloC'><b>Total clientes </b></p>"
+"<p class='pTituloC' >"+ document.getElementById("inptRegistroNroHistorialInformeCredito").value+"</p>"
+"</td>"
+"<td style='width:20%;text-align:left'>"
+"<p class='pTituloC'><b>Total  a cobrar</b></p>"
...
document.getElementById("tbDatosImpresiones").innerHTML=document.getElementById("table_Informe_Credito").innerHTML
}
 
if (ventana == "informegralproductos") {
...
+"<td style='width:20%;text-align:left'>"
+"<p class='pTituloC'><b>Local:</b></p>"
+"<p class='pTituloC' >"+$("select[id=inptlocalProductoBuscarinformegralproductos]").children(":selected").text()+"</p>"
+"</td>"
+"<td style='width:20%;text-align:left'>"
+"<p class='pTituloC'><b>Existencia:</b></p>"
+"<p class='pTituloC' >"+$("select[id=inptExistenciaProductoBuscarinformegralproductos]").children(":selected").text()+"</p>"
+"</td>"
+"<td style='width:20%;text-align:left'>"
...
+"</td>"
+"</tr>"
+"</table><br><br><center><h1 class='pTituloD' >INFORME GENERAL DE PRODUCTOS</h1><br></center>"
paginaPie =
"<br><br><table class='TableRepor0' style='width:100%'>"
+"<tr>"
+"<td style='width:20%;text-align:left'>"
+"<p class='pTituloC'><b>Total Registro </b></p>"
+"<p class='pTituloC' >"+ document.getElementById("inptTotalRegistroinformegralproductos").value+"</p>"
+"</td>"
+"<td style='width:20%;text-align:left'>"
+"<p class='pTituloC'><b>Total  Costo</b></p>"
+"<p class='pTituloC' >"+document.getElementById("inptTotalRegistroProductosCostoinformegralproductos").value+"</p>"
+"</td>"
+"<td style='width:20%;text-align:left'>"
+"<p class='pTituloC'><b>Total Stock</b></p>"
+"<p class='pTituloC' >"+document.getElementById("inptTotalStockinformegralproductos").value+"</p>"
+"</td>"
+"<td style='width:60%;text-align:left'>"
+"</td>"
+"</tr>"
...
document.getElementById("divCabeceraImpresiones").innerHTML=pagina
document.getElementById("divPieImpresiones").innerHTML=paginaPie
document.getElementById("tbTitulosImpresiones").innerHTML=document.getElementById("tdTituloImpreinformegralproductos").innerHTML
document.getElementById("tbDatosImpresiones").innerHTML=document.getElementById("table_productos_informegralproductos").innerHTML
}
...
+"<p class='pTituloC' >"+ document.getElementById("inptBuscarCuentasAcobrarinforme").value+"</p>"
+"</td>"
+"<td style='width:20%;text-align:left'>"
+"<p class='pTituloC'><b>Fecha de Impresión</b></p>"
+"<p class='pTituloC' >"+fechaimpresion+"</p>"
+"</td>"
+"</tr>"
+"</table><center><h1 class='pTituloD' >CUENTAS PENDIENTES</h1></center>"
paginaPie =
"<table class='TableRepor0' style='width:100%'>"
+"<tr>"
+"<td style='width:20%;text-align:left'>"
...
if (ventana == "catalago") {
	if(controlacceso("IMPRIMIRCATALOGO","accion")==false){ return;}
	pagina =
"<table class='TableRepor0' style='width:100%'>"
+"<tr>"
+"<td style='width:20%;text-align:left'>"
+"<p class='pTituloC'><b>Categoria</b></p>"
+"<p class='pTituloC' >"+$("select[id=inptCategoriaProductoCatalogo]").children(":selected").text()+"</p>"
+"</td>"
+"<td style='width:20%;text-align:left'>"
+"<p class='pTituloC'><b>Marca</b></p>"
+"<p class='pTituloC' >"+$("select[id=inptMarcaProductoCatalogo3]").children(":selected").text()+"</p>"
+"</td>"
+"<td style='width:20%;text-align:left'>"
+"<p class='pTituloC'><b>Existencia</b></p>"
+"<p class='pTituloC' >"+$("select[id=inptStockProductoCatalogo]").children(":selected").text()+"</p>"
+"</td>"
+"<td style='width:20%;text-align:left'>"
+"<p class='pTituloC'><b>Producto</b></p>"
+"<p class='pTituloC' >"+document.getElementById("inptProveedorProductoCatalogo").value+"</p>"
+"</td>"
+"<td style='width:20%;text-align:left'>"
+"<p class='pTituloC'><b>Usuario</b></p>"
+"<p class='pTituloC' >"+document.getElementById("ptituloUser2").textContent+"</p>"
+"</td>"
+"<td style='width:20%;text-align:left'>"
+"<p class='pTituloC'><b>Fecha de impresión</b></p>"
+"<p class='pTituloC' >"+fechaimpresion+"</p>"
+"</td>"
+"</tr>"
+"</table><br><br><center><h1 class='pTituloD' >CATALOGO</h1><br></center>"
...
var miElemento = document.getElementById("tbDatosImpresiones");
miElemento.classList.add("display_grid");
}
...
+"<tr>"
+"<td style='width:20%;text-align:left'>"
+"<p class='pTituloC'><b>Total Regiatros </b></p>"
+"<p class='pTituloC' >"+ document.getElementById("inptTotalRegistroInventario").value+"</p>"
+"</td>"
+"<td style='width:20%;text-align:left'>"
+"<p class='pTituloC'><b>Total Costo</b></p>"
+"<p class='pTituloC' >"+document.getElementById("inptTotalRegistroProductosCostoInventario").value+"</p>"
+"</td>"
+"<td style='width:60%;text-align:left'>"
+"</td>"
+"</tr>"
...
+"</td>"
+"</tr>"
+"</table><br><br><center><h1 class='pTituloD' >DEPÓSITO</h1><br></center>"


document.getElementById("divCabeceraImpresiones").innerHTML=pagina
document.getElementById("tbTitulosImpresiones").innerHTML=document.getElementById("tdTituloImpreInformeDeposito").innerHTML
document.getElementById("tbDatosImpresiones").innerHTML=document.getElementById("table_informe_deposito").innerHTML
}
...
+"<td style='width:20%;text-align:left'>"
+"<p class='pTituloC'><b>Fecha Inicio:</b></p>"
+"<p class='pTituloC' >"+$("#inptBuscarInformeDeudaClienteF1").val()+"</p>"
+"</td>"
+"<td style='width:20%;text-align:left'>"
+"<p class='pTituloC'><b>Fecha Fin:</b></p>"
+"<p class='pTituloC'  >"+$("#inptBuscarInformeDeudaClienteF2").val()+"</p>"
+"</td>"
+"<td style='width:20%;text-align:left'>"
+"<p class='pTituloC'><b>Tipo</b></p>"
+"<p class='pTituloC'  >"+$("select[id=selectClienteFijoDeudaCliente]").children(":selected").text()+"</p>"
+"</td>"
+"<td style='width:40%;text-align:left'>"

+"</td>"
+"</tr>"
+"</table><br><br><center><h1 class='pTituloD' >DEUDA CLIENTE</h1><br></center>"
document.getElementById("divCabeceraImpresiones").innerHTML=pagina
document.getElementById("tbTitulosImpresiones").innerHTML=document.getElementById("tbDeudaCliente").innerHTML
document.getElementById("tbDatosImpresiones").innerHTML=document.getElementById("table_informe_deuda_cliente").innerHTML
}
...
"<table class='TableRepor0' style='width:100%'>"
+"<tr>"
+"<td style='width:20%;text-align:left;display:none'>"
+"<p class='pTituloC'><b>Local:</b></p>"
+"<p class='pTituloC' >"+$("select[id=inptlocalProductoBuscarStock]").children(":selected").text()+"</p>"
+"</td>"
+"<td style='width:20%;text-align:left'>"
+"<p class='pTituloC'><b>Existencia:</b></p>"
+"<p class='pTituloC'  >"+$("select[id=inptExistenciaProductoBuscarstock]").children(":selected").text()+"</p>"
+"</td>"
+"<td style='width:60%;text-align:left'>"

+"</td>"
+"</tr>"
+"</table><br><br><center><h1 class='pTituloD' >PRODUCTO FALTANTE</h1><br></center>"
document.getElementById("divCabeceraImpresiones").innerHTML=pagina
document.getElementById("tbTitulosImpresiones").innerHTML=document.getElementById("tdTituloImpreStock").innerHTML
document.getElementById("tbDatosImpresiones").innerHTML=document.getElementById("table_comision_productosStock_2").innerHTML
}


if (ventana == "pagosproveedor") {
...
+"</td>"
+"</tr>"
+"</table><br><br><center><h1 class='pTituloD' >PAGOS A PROVEEDOR</h1><br></center>";
...
+"<td style='width:20%;text-align:left'>"
+"<p class='pTituloC'><b>Total </b></p>"
+"<p class='pTituloC' >"+ document.getElementById("inputTotalPagarProveedor").value+"</p>"
+"</td>"
...
document.getElementById("divCabeceraImpresiones").innerHTML=pagina
document.getElementById("tbTitulosImpresiones").innerHTML=document.getElementById("tdTituloPagosAProveedor").innerHTML
document.getElementById("tbDatosImpresiones").innerHTML=document.getElementById("table_abm_PagoProveedorDetalle_imprimir").innerHTML
document.getElementById("divPieImpresiones").innerHTML=paginaPie
}
...
+"</table>"
	...
	document.getElementById('TitleRepot').innerHTML="CARTERA DE CLIENTES";
	document.getElementById("divCabezado").innerHTML=pagina
	document.getElementById("DivTablas").innerHTML=document.getElementById("table_Informe_Credito").innerHTML;
	document.getElementById("DivPiePagina").innerHTML=paginaPie;
...
function manejadordeerroresjquery(error,textStatus,funcion){
    ...
    ver_vetana_informativa('TIPO DE CONTENIDO NO SOPORTADO. VERIFIQUE LA PETICION ENVIADA AL SERVIDOR');
  ...
  ver_vetana_informativa('ERROR, TIEMPO AGOTADO');
    ...
    ver_vetana_informativa('ERROR HTTP: ' + error);
...
+"<td style='width:33%;text-align:left'>"
+"<p class='pTituloC' style='text-align:center'><b>-------------------------------------</b></p>"
+"<p class='pTituloC' style='text-align:center'><b>VERIFICADOR</b></p>"
+"</td>"
+"</tr>"
+"</table>"
...
document.getElementById("DivImprimir").innerHTML=ficha;
   var documento= document.getElementById("DivImprimir").innerHTML;
     localStorage.setItem("reporte", documento);
	   localStorage.setItem("tipo", "ticket");
	 window.open("/GoodVentaElectroCasaMaric/system/reportRecibo.html");
	 document.getElementById("DivImprimir").innerHTML = "";
...
function ReImprimirDivTickeFacturaPago( Fecha, Cajero, CuotasNro, Pagado, DiasAtrazado,  NombreCliente, CiCliente, NroRecibo, tipoventa,  totalInteres, deudaActual, totalpagado, totaldescuento,  TotalVenta, InteresActual, deudaActualsininteres) {
  ...
  <div class="ticket-wrap">
    ...
    <div class="section">
      <div class="section-title">
        ...
        <div class="pill">Pago registrado</div>
      </div>
  ...
  ">
    ...
    <div>
      ...
      <div class="v">${NombreCliente}</div>
    </div>

    <div>
      <div class="k">Fecha</div>
      <div class="v">${Fecha}</div>
    </div>

    <!-- Columna izquierda -->
    <div>
      <div class="k">RUC / C.I.</div>
      <div class="v">${CiCliente}</div>
    </div>

    <div>
      <div class="k">Cajero</div>
      <div class="v">${Cajero}</div>
    </div>
      ...
      <table class="tbl">
        ...
        <tbody>
          ${typeof paginaticket !== "undefined" ? paginaticket : ""}
        </tbody>
      </table>
      ...
      <div class="layout-2col">
        <div>
          <div class="section-title" style="margin-bottom:8px;">
            <h2>Cuotas</h2>
            <div class="pill">Vencimientos</div>
          </div>

          <table class="tbl">
            <thead>
              <tr>
                <th style="width:120px">FECHA VENC.</th>
                <th>DESCRIPCION</th>
				<th>TIPO</th>
        ...
        <div class="totals">
          ...
          <div class="row">
            ...
            <div class="value">${TotalVenta} Gs.</div>
          </div>

          <div class="row">
            <div class="label">Total pagado</div>
            <div class="value">${totalpagado} Gs.</div>
          </div>

          <div class="row">
            <div class="label">Total Importe:</div>
            <div class="value">${totalCobroPagado} Gs.</div>
          </div>

          <div class="row">
            <div class="label">Saldo actual</div>
            <div class="value">${deudaActual} Gs.</div>
          </div>
    ...
    <div class="ticket-footer">
      <div class="sign">
        <div class="line"></div>
        <div class="hint">Firma y Aclaración</div>
      </div>

      <div class="thanks">
        Documento generado por el sistema.<br>
  ...
  // Render + almacenar + abrir impresión
  document.getElementById("DivImprimir").innerHTML = pagina;

  const documento = document.getElementById("DivImprimir").innerHTML;
  localStorage.setItem("reporte", documento);
  localStorage.setItem("tipo", "ticket");
...
function formatearNumero(totalCobroPagado) {
    // Eliminar todo lo que no sea número
    totalCobroPagado = totalCobroPagado.toString().replace(/\D/g, '');

    // Convertir a número
    let numero = parseInt(totalCobroPagado, 10) || 0;
...
function ReImprimirDivTickeFacturaPagoTicket(Fecha,Cajero,CuotasNro,Pagado,DiasAtrazado,NombreCliente,CiCliente,NroRecibo,tipoventa,totalInteres,deudaActual,totalpagado,totaldescuento,TotalVenta,InteresActual,deudaActualsininteres,detalleventaticket){
...
+"</tr>"
+"</table>"
+"<div class='divSeparadorTicket' style='margin-top:5px;margin-bottom:5px;display:none' ></div>"
+"<table class='tableTicket'>"
+"<tr>"
+"<td style='width:60px'><b>Cajero :</b></td>"
+"<td style=''>"+Cajero+"</td>"
...
ReImprimirDivTickeFacturaPago( fechaVenta, cajera, CuotasNro, TotalPagado,"0",  NombreRecibo, DocumentoRecibo, NroVentas, "CONTADO",  "0", "", TotalPagado, TotalDescuentoRecibo,  totalesRecibo, "0", "0")
...
rellenarCampo('ContratoTelefono', datosContrato.telefono);
rellenarCampo('ContratoNombre', datosContrato.cliente);
rellenarCampo('ContratoCod_venta', datosContrato.cod_venta);
rellenarCampo('ContratototalVenta', datosContrato.totalVenta+" Gs.");
rellenarCampo('ContratoEntrega', datosContrato.entrega+" Gs.");
rellenarCampo('ContratoSaldo', datosContrato.saldo+" Gs.");
rellenarCampo('ContratoCantidadCuotas', datosContrato.plazo);
rellenarCampo('ContratoCuotaMensual', datosContrato.mensual+" Gs.");
rellenarCampo('ContratoDetalleVenta', datosContrato.detalle);
 
rellenarCampo('ContratoClienteFirma', datosContrato.cliente); 
rellenarCampo('ContratoCiFirma', "C.I. Nº:"+datosContrato.ruc+" - COMPRADOR"); 
 
 var documento= document.getElementById("DivContrato").innerHTML;
     localStorage.setItem("reporte", documento);
	   localStorage.setItem("tipo", "reporte");
	
	  var URL= "/GoodVentaElectroCasaMaric/system/contrato.html"
        window.open(URL, 'Imprimir', 'toolbar=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=1,left = 0');
}
