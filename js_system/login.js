...
function verificardatos(){
	...
	var inptcajauser=document.getElementById('inptcajauser').value
	 if(user==""){
...
function entrar_al_sistema(datos1,datos2,datos3){
	...
	var navegador=obtener_navegor_en_uso()
	 var datos = new FormData();
			...
			data: datos,
			url: "/GoodVentaElectroCasaMaric/php_system/login.php",
			type:"post",
	        cache:false,
			contentType: false,
			processData: false,
...
var caja=document.getElementById("inptcajauser").value;
  ...
  window.location="/GoodVentaElectroCasaMaric/system/inicio.html?p="+p+"&q="+u+"&c="+caja;
...
function ver_vetana_informativa(titulo,id_c){
	...
	$("div[id=capa_informativa]").fadeOut(2000)
}
